		...
		namespace CNTK {
			...
			*/
			...
			private:
                ...
                Matrix<ElemType> mLogOfRight;

			public:
				SegmentalEvaluateNode(const short deviceId = AUTOPLACEMATRIX, const std::wstring name = L"")
                    : ComputationNode(deviceId), mAlpha(deviceId), mBeta(deviceId), mPostProb(deviceId), mLogOfRight(deviceId)
				{
				...
				{
                    EvaluateThisNodeS(mPostProb, mAlpha, mBeta, FunctionValues(), Inputs(0)->FunctionValues(), Inputs(1)->FunctionValues(), Inputs(2)->FunctionValues(), mLogOfRight);
				}
				...
				static void EvaluateThisNodeS(Matrix<ElemType>& postprob, Matrix<ElemType>& alpha, Matrix<ElemType>& beta, Matrix<ElemType>& functionValues, const Matrix<ElemType>& inputFunctionValues0, const Matrix<ElemType>& inputFunctionValues1, const Matrix<ElemType>& inputFunctionValues2, Matrix<ElemType>& logOfRight);
			...
			/**
			* implement the linear chain forward backward algorithm
			*/
			template<class ElemType>
			class LinearChainForwardBackwardEvaluateNode : public SegmentalEvaluateNode<ElemType>
			{
				virtual const std::wstring OperationName() const { return TypeName(); }
				static const std::wstring TypeName() { return L"LinearChainForwardBackwardEvaluateNode"; }
			};
