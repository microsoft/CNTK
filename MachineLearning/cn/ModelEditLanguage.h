...
public:
    typedef map<ComputationNode<ElemType>*,ComputationNode<ElemType>*> MapNodes;
    ...
    vector<ComputationNode<ElemType>*> FindSymbols(const std::string& symbol, NetNdl<ElemType>*& netNdl)
    {
        ...
        vector<ComputationNode<ElemType>*> nodes = cn->GetNodesFromName(name);
        ...
        if (nodes.empty() && netNdl->ndl != nullptr)
        {
            ...
            if (ndlNode != nullptr)
            {
                ComputationNode<ElemType>* value = (ComputationNode<ElemType>*)ndlNode->GetEvalValue();
                if (value != nullptr)
                {
                    nodes.push_back((ComputationNode<ElemType>*)ndlNode->GetEvalValue());
                }
                else
                {
    ...
    typedef pair<ComputationNode<ElemType>*,std::wstring> GenNameValue;
    vector<GenNameValue> GenerateNames(const std::string& symbolIn, const std::string& symbolOut, NetNdl<ElemType>*& netNdlIn, NetNdl<ElemType>*& netNdlOut)
    {
        ...
        {
            ComputationNode<ElemType>* nodeIn = nodes[0];
            vector<ComputationNode<ElemType>*> nodesOut = netNdlOut->cn->GetNodesFromName(nameOut);
    ...
    void CopyNodes(const std::string& symbolIn, const std::string& symbolOut, CopyNodeFlags copyFlags)
    {
        ...
        for (GenNameValue name : copyNodes)
        {
            ...
            ComputationNode<ElemType>* newNode = netNdlTo->cn->CopyNode(*netNdlFrom->cn, nodeName, nodeOutName, copyFlags);
    ...
    void CopySubTree(const std::string& symbolFrom, const std::string& toCNName, const std::string toNamePrefix, CopyNodeFlags copyFlags)
    {
        ...
        vector<ComputationNode<ElemType>*> fromNodes = FindSymbols(symbolFrom, netNdlFrom);
        size_t firstStart, firstCount, secondStart, secondCount;
        ...
        {
            ComputationNode<ElemType>* fromNode = fromNodes[i];
