...
namespace
{
    ...
    void LoopBasedOnSamples(const std::wstring& name, const DeviceDescriptor& device, std::function<DistributedLearnerPtr(LearnerPtr)> factory, const FeedForwardClassifier& classifier)
    {
        ...
        auto trainer = CreateTrainer(classifier.output, classifier.trainingLoss, classifier.prediction, { factory({ SGDLearner(classifier.output->Parameters(), LearningRateSchedule(learningRatePerSample, 1)) }) });
...
void TestDistributedCheckpointing()
{
    ...
    for (auto device : devices)
    {
        ...
        auto learner = SGDLearner(ff.output->Parameters(), LearningRateSchedule(0.02, 1));
