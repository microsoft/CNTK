...
if [ "\$TEST_DEVICE" = "gpu" ]; then
  ...
  for f in *.ipynb; do
    ...
    if [[ \$f != CNTK_203_Reinforcement_Learning_Basics.ipynb && \$f != CNTK_204_Sequence_To_Sequence.ipynb ]]; then
      jupyter nbconvert --debug --to notebook --execute --ExecutePreprocessor.kernel_name=python\$(python -c "import sys; print(sys.version_info[0])") --ExecutePreprocessor.timeout=2700 --output - \$f
    fi
  done
fi
