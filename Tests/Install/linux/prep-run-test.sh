...
if [ "\$TEST_DEVICE" = "gpu" ]; then
  ...
  for f in *.ipynb; do
    ...
    # 104: can't get data
    # 204: interactive
    if [[ \$f != CNTK_203_Reinforcement_Learning_Basics.ipynb && \$f != CNTK_104_Finance_Timeseries_Basic_with_Pandas_Numpy.ipynb && \$f != CNTK_204_Sequence_To_Sequence.ipynb ]]; then
