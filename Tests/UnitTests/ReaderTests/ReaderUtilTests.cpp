...
static void CreateTestFile(const std::string& content, const std::wstring filename = L"test.tmp")
{
    ...
    WriteOrDie(content.c_str(), content.size(), 1, OpenOrDie(filename, L"w+b"));
}
...
BOOST_AUTO_TEST_SUITE(TextInputIndexBuilderTests)
...
static IndexBuilderPtr GetIndexBuilder(const std::string& input)
{
    ...
    auto f = OpenOrDie(filename, L"rb").release();

    auto testFileSize = input.size();
    BOOST_REQUIRE_EQUAL(testFileSize, filesize(f));

    return unique_ptr<TextInputIndexBuilder, std::function<void(TextInputIndexBuilder*)>>(new TextInputIndexBuilder(filename, f),
        [f, filename](TextInputIndexBuilder* builder)
    {
        delete builder;
        fclose(f);
        _wunlink(filename.c_str());
...
BOOST_AUTO_TEST_CASE(Index_with_caching)
{
    ...
    auto f1 = OpenOrDie(L"test.tmp", L"rb");

    auto index = TextInputIndexBuilder(L"test.tmp", f1.get()).Build();
    // Cache is written out asynchronously in a separate thread, 
    Sleep(1000);  // sleep for a second to give enought time to finish writing.

    auto cachedIndex = TextInputIndexBuilder(L"test.tmp", nullptr).Build();
...
BOOST_AUTO_TEST_CASE(Index_64MB_with_caching_check_perf)
{
    ...
    bool checkIndicesAreIdentical = true;

    for (int i = 0; i < 10; i++) 
    {
        wstring filename = std::to_wstring(i) + L".perf.test.tmp";
        
        CreateTestFile(content, filename);

        auto f1 = OpenOrDie(filename, L"rb");
        DWORD start = GetTickCount();
        auto index = TextInputIndexBuilder(filename, f1.get()).SetChunkSize(g_1MB >> 4).Build();
        DWORD end = GetTickCount();
        timeToBuildIndexFromFile += (end - start);
        // Cache is written out asynchronously in a separate thread, 
        Sleep(1000);  // sleep for a second to give enought time to finish writing.
        start = GetTickCount();
        auto cachedIndex = TextInputIndexBuilder(filename, nullptr).SetChunkSize(g_1MB >> 4).Build();
        end = GetTickCount();
        timeToBuildIndexFromCache += (end - start);

        _wunlink(filename.c_str());

        if (checkIndicesAreIdentical) 
        {
            CheckIdentical(index, cachedIndex);
            checkIndicesAreIdentical = false;
        }
    }
...
BOOST_AUTO_TEST_CASE(Index_1GB_with_caching_check_perf)
{
    ...
    wstring filename = L"1gb.perf.test.tmp";

    CreateTestFile(content, filename);

    auto f1 = OpenOrDie(filename, L"rb");
    DWORD start = GetTickCount();
    auto index = TextInputIndexBuilder(filename, f1.get()).Build();
    DWORD end = GetTickCount();
    timeToBuildIndexFromFile += (end - start);
    // Cache is written out asynchronously in a separate thread, 
    Sleep(5000);  // sleep for a few seconds to give enought time to finish writing.
    start = GetTickCount();
    auto cachedIndex = TextInputIndexBuilder(filename, nullptr).Build();
    end = GetTickCount();
    timeToBuildIndexFromCache += (end - start);
