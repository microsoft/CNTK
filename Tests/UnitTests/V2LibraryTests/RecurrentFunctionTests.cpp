                          ...
                          unsigned int seed = 1)
{
    ...
    Parameter plusParam(MakeReferenceCountedObject<NDArrayView>((ElementType)0.1, std::initializer_list<size_t>({ outputDim }), device));
    ...
    for (size_t iterIdx = 0; iterIdx < numIterations; ++iterIdx)
    {
        ...
        else
        {
            ...
            NDArrayViewPtr inputValueData = MakeReferenceCountedObject<NDArrayView>(inputShape, inputData.data(), inputData.size(), DeviceDescriptor::CPUDevice(), true);
            if (useSparseInputs)
            {
                NDArrayViewPtr sparseInputValueData = MakeReferenceCountedObject<NDArrayView>(AsDataType<ElementType>(), StorageFormat::SparseCSC, inputShape, DeviceDescriptor::CPUDevice());
                sparseInputValueData->CopyFrom(*inputValueData);
                inputValueData = sparseInputValueData->Alias(true);
            }

            NDMaskPtr inputMask = MakeReferenceCountedObject<NDMask>(NDShape({ maxActualSequenceLength, numSequences }), DeviceDescriptor::CPUDevice());
            for (size_t i = 0; i < numSequences; ++i)
                inputMask->MaskSection({ sequenceLengths[i], i }, { NDShape::InferredDimension, 1 });

            inputValue = MakeReferenceCountedObject<Value>(inputValueData, inputMask);
        }

        NDShape reducedOutputShape = {};
        std::vector<ElementType> reducedOutputData(reducedOutputShape.TotalSize());
        ValuePtr reducedOutputValue = MakeReferenceCountedObject<Value>(MakeReferenceCountedObject<NDArrayView>(reducedOutputShape, reducedOutputData.data(), reducedOutputData.size(), DeviceDescriptor::CPUDevice(), false));
        ...
        ValuePtr plusOutputValue = MakeReferenceCountedObject<Value>(MakeReferenceCountedObject<NDArrayView>(plusOutputShape, plusOutputData.data(), plusOutputData.size(), DeviceDescriptor::CPUDevice(), false), MakeReferenceCountedObject<NDMask>(inputValue->Mask()->Shape(), inputValue->Mask()->Device()));
        ...
        ValuePtr rootGradientValue = MakeReferenceCountedObject<Value>(MakeReferenceCountedObject<NDArrayView>(plusOutputShape, rootGradientsData.data(), rootGradientsData.size(), DeviceDescriptor::CPUDevice(), true), inputValue->Mask()->DeepClone());
        ...
        ValuePtr plusParameterGradientValue = MakeReferenceCountedObject<Value>(MakeReferenceCountedObject<NDArrayView>(plusParam.Shape(), plusParameterGradientData.data(), plusParameterGradientData.size(), DeviceDescriptor::CPUDevice(), false));
        ValuePtr timesParameterGradientValue = MakeReferenceCountedObject<Value>(MakeReferenceCountedObject<NDArrayView>(timesParam.Shape(), timesParameterGradientData.data(), timesParameterGradientData.size(), DeviceDescriptor::CPUDevice(), false));
        ValuePtr inputGradientValue = MakeReferenceCountedObject<Value>(MakeReferenceCountedObject<NDArrayView>(inputShape, inputGradientData.data(), inputGradientData.size(), DeviceDescriptor::CPUDevice(), false), inputValue->Mask()->DeepClone());
