...
BOOST_AUTO_TEST_CASE(CheckModelVersion)
{
    ...
    // Instead, please do notify the CNTK release team (AlexyO, Wolfgang, Zhou, Mark) to prepare required steps for the next release.
    BOOST_REQUIRE_MESSAGE(CURRENT_CNTK_MODEL_VERSION == 17, "The model version has been changed. Before making changes in this test, please first notify the CNTK release team to prepare required steps in the next release. Thanks!\n");
}
