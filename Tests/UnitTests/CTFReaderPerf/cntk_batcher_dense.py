...
def create_reader(path, is_training):
    return C.io.MinibatchSource(C.io.CTFDeserializer(path, C.io.StreamDefs(
        features = C.io.StreamDef(field='a', shape=input_vocab_dim, is_sparse=False),
        labels   = C.io.StreamDef(field='b', shape=label_vocab_dim, is_sparse=False)
    )), randomize = True, max_sweeps = 1, multithreaded_deserializer=True)

# Train data reader
reader = create_reader('dense_2M_seqs.ctf', True)
...
batch_only(reader, max_epochs=1, epoch_size=61022956)#2000000)
