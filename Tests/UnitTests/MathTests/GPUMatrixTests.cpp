...
BOOST_FIXTURE_TEST_CASE(GPUMatrixStraightThroughForward, RandomSeedFixture)
{
    ...
    {
        float binary_val = value_carray[i] <= 0 ? -1.f : 1.f;
        BOOST_CHECK_EQUAL(result_carray[i], binary_val);
    }
}

BOOST_FIXTURE_TEST_CASE(GPUMatrixStraightThroughBackward, RandomSeedFixture)
{
    const float low = -5.f;
    const float high = 5.f;
    const size_t nRows = 100;
    const size_t nCols = 50;
    auto value = GPUMatrix<float>::RandomUniform(nRows, nCols, c_deviceIdZero, low, high, IncrementCounter());
    auto outgrad = GPUMatrix<float>::RandomUniform(nRows, nCols, c_deviceIdZero, low, high, IncrementCounter());
    auto result = GPUMatrix<float>::RandomUniform(nRows, nCols, c_deviceIdZero, low, high, IncrementCounter());
    GPUMatrix<float> ingrad(nRows, nCols, c_deviceIdZero);
    GPUMatrix<float>::StraightThroughBackward(value, result, outgrad, ingrad);
    unique_ptr<float[]> value_carray(value.CopyToArray());
    unique_ptr<float[]> outgrad_carray(outgrad.CopyToArray());
    unique_ptr<float[]> ingrad_carray(ingrad.CopyToArray());

    for (int i = 0; i < nRows * nCols; ++i)
    {
        float ingrad_check = abs(value_carray[i]) <= 1 ? outgrad_carray[i] : 0.f;
        BOOST_CHECK_EQUAL(ingrad_carray[i], ingrad_check);
    }
}
