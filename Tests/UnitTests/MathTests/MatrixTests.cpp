        ...
        namespace CNTK
        {
            namespace Test
            {
                ...
                BOOST_FIXTURE_TEST_CASE(MatrixElementOps, RandomSeedFixture)
                {
                    ...
                    SingleMatrix m00(2, 3, AUTOPLACEMATRIX);
                    m00(0, 0) = 1.0f; m00(0, 1) = static_cast<float> (1 / 2.0); m00(0, 2) = static_cast<float> (1 / 3.0);
                    m00(1, 0) = static_cast<float> (1 / 4.0); m00(1, 1) = static_cast<float>(1 / 5.0); m00(1, 2) = static_cast<float>(1 / 6.0);
                    ...
                    m2(0, 0) = 0.7311f; m2(0, 1) = 0.8808f; m2(0, 2) = 0.9526f;
                    m2(1, 0) = 0.9820f; m2(1, 1) = 0.9933f; m2(1, 2) = 0.9975f;
                    ...
                    m3.SetValue(m0);
                    m3.InplaceTanh();
                    m2(0, 0) = 0.7616f; m2(0, 1) = 0.9640f; m2(0, 2) = 0.9951f;
                    m2(1, 0) = 0.9993f; m2(1, 1) = 0.9999f; m2(1, 2) = 1.0000f;
                    BOOST_CHECK(m3.IsEqualTo(m2, c_epsilonFloatE4));

                    m3.SetValue(m0);
                    m3.InplaceLogSoftmax(true);
                    m3.InplaceExp();
                    m2(0, 0) = 0.0474f; m2(0, 1) = 0.0474f; m2(0, 2) = 0.0474f;
                    m2(1, 0) = 0.9526f; m2(1, 1) = 0.9526f; m2(1, 2) = 0.9526f;
                    BOOST_CHECK(m3.IsEqualTo(m2, c_epsilonFloatE4));

                    m3.SetValue(m0);
                    m3.InplaceLogSoftmax(false);
                    m3.InplaceExp();
                    m2(0, 0) = 0.0900f; m2(0, 1) = 0.2447f; m2(0, 2) = 0.6652f;
                    m2(1, 0) = 0.0900f; m2(1, 1) = 0.2447f; m2(1, 2) = 0.6652f;
                    BOOST_CHECK(m3.IsEqualTo(m2, c_epsilonFloatE4));

                    m3.SetValue(m0);
                    m3.InplaceHardmax(true);
                    m2(0, 0) = 0.0f; m2(0, 1) = 0.0f; m2(0, 2) = 0.0f;
                    m2(1, 0) = 1.0f; m2(1, 1) = 1.0f; m2(1, 2) = 1.0f;
                    BOOST_CHECK(m3.IsEqualTo(m2, c_epsilonFloatE4));

                    m3.SetValue(m0);
                    m3.InplaceSqrt();
                    m2(0, 0) = 1.0f; m2(0, 1) = 1.4142f; m2(0, 2) = 1.7321f;
                    m2(1, 0) = 2.0f; m2(1, 1) = 2.2361f; m2(1, 2) = 2.4495f;
                    BOOST_CHECK(m3.IsEqualTo(m2, c_epsilonFloatE4));

                    m3.SetValue(m0);
                    m3.InplaceExp();
                    m2(0, 0) = 2.7183f; m2(0, 1) = 7.3891f; m2(0, 2) = 20.0855f;
                    m2(1, 0) = 54.5982f; m2(1, 1) = 148.4132f; m2(1, 2) = 403.4288f;
                    BOOST_CHECK(m3.IsEqualTo(m2, c_epsilonFloatE4));
