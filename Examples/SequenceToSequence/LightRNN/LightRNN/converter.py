...
def load_vocab_from_file(vocab_file):
    ...
    vocab = {}
    with open(vocab_file, encoding=TEXT_ENCODING) as input_file:
...
def load_vocab_location_from_file(location_file):
    ...
    location = {}
    with open(location_file, encoding=TEXT_ENCODING) as input_file:
        row_id = 0
...
def generate_vocab_from_source_file(input_dir, freq=None):
    ...
    word_count = {}
    for file in files:
        with open(file, encoding=TEXT_ENCODING) as input_file:
            for line in input_file:
...
def save_to_vocab_file(word_index, vocab_file):
    ...
    with open(vocab_file, 'w', encoding=TEXT_ENCODING) as input_file:
        for word in word_index:
...
def save_to_vocab_location_file(word_location, vocab_location_file):
    ...
    sqrt_length = ceil(sqrt(vocab_size))
    locations = {}
    ...
    with open(vocab_location_file, 'w') as output_file:
        for i in range(sqrt_length):
            for j in range(sqrt_length):
