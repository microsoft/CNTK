...
lr_per_sample    = 0.001
lr_schedule      = C.learning_rate_schedule(lr_per_sample, ref_minibatch_size=1)
...
def adjust_lr_callback(index, average_error, cv_num_samples, cv_num_minibatches):
    ...
    if (prev_metric - average_error) / prev_metric < 0.05: # relative gain must reduce metric by at least 5% rel
        learner.reset_learning_rate(C.learning_rate_schedule(learner.learning_rate() / 2, ref_minibatch_size=1))
