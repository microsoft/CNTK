...
def LocalResponseNormalization(k, n, alpha, beta, name=''):
    ...
    W = C.constant(alpha/(2*n+1), (1,2*n+1,1,1), name='W', dtype=dtype)
...
def create_alexnet():
    ...
    with C.default_options(dtype=dtype):
        ...
        # remove mean value 
        mean_removed_features = minus(feature_var, constant(114, dtype=dtype), name='mean_removed_input')
...
def alexnet_train_and_eval(train_data, test_data, num_quantization_bits=32, block_size=3200, warm_up=0, minibatch_size=256, epoch_size = 1281167, max_epochs=112,
    ...
    #train_and_test_fake(network, trainer, minibatch_size, epoch_size)
    ...
    for epoch in range(max_epochs):       # loop over epochs
        ...
        fake_mb = {network['feature']:fake_feature, network['label']:fake_label}
        while sample_count < epoch_size:  # loop over minibatches in the epoch
            trainer.train_minibatch(fake_mb)
            sample_count += minibatch_size                     # count samples processed so far
...
if __name__=='__main__':
    ...
    parser.add_argument('-n', '--num_epochs', help='Total number of epochs to train', type=int, required=False, default='6')
    parser.add_argument('-m', '--minibatch_size', help='Minibatch size', type=int, required=False, default='32')
    parser.add_argument('-e', '--epoch_size', help='Epoch size', type=int, required=False, default='3200')
    ...
    parser.add_argument('-fp32', '--fp32', help='float32 mode', action='store_true')
