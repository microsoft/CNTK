...
def create_reader(map_file, train, total_number_of_samples, distributed_after=cntk.io.INFINITE_SAMPLES):
    ...
    if train:
        transforms += [
            cntk.io.ImageDeserializer.crop(crop_type='randomside', crop_ratio=0.88671875, jitter_type='uniratio') # train uses jitter
        ]
    else: 
        transforms += [
            cntk.io.ImageDeserializer.crop(crop_type='center', ratio=0.88671875) # test uses center crop (from 256x256 crop to 227x227)
        ]
...
def alexnet_imagenet1k(create_train_reader, test_reader, create_dist_learner, max_epochs=80, log_to_file=None, num_mbs_per_log=None, gen_heartbeat=False):
    ...
    with cntk.layers.default_options(activation=cntk.ops.relu, pad=True): 
        z = cntk.models.Sequential([
            cntk.layers.Convolution((11,11), 96, init=normal(0.01), pad=False, strides=4, name='conv1'), 
            LRN()
    ...
    # training config
    epoch_size = 1281167  # for now we manually specify epoch size
    minibatch_size = 256

    # Set learning parameters
    lr_per_minibatch  = [0.1]*25 + [0.01]*25 + [0.001]*25 + [0.0001]*25 + [0.00001]
    lr_schedule       = cntk.learner.learning_rate_schedule(lr_per_minibatch, unit=cntk.learner.UnitType.minibatch, epoch_size=epoch_size)
    mm_schedule       = cntk.learner.momentum_schedule(0.9, epoch_size=epoch_size)
    l2_reg_weight     = 0.002
...
if __name__=='__main__':
    ...
    alexnet_imagenet1k(create_train_reader, test_reader, create_dist_learner, log_to_file=log_dir, num_mbs_per_log=10, gen_heartbeat=False)
