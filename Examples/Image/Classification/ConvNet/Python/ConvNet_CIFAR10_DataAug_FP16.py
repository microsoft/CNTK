...
feature = cntk.input_variable((num_channels, image_height, image_width))
label = cntk.input_variable((num_classes))
cast = cntk.combine(cntk.cast(feature, dtype=np.float16), cntk.cast(label, dtype=np.float16))
...
def train_model(reader, model, criterion, epoch_size=50000, max_epochs=80):
    ...
    for epoch in range(max_epochs):       # loop over epochs
        ...
        while sample_count < epoch_size:  # loop over minibatches in the epoch
            mb = reader.next_minibatch(min(minibatch_size, epoch_size - sample_count), input_map=input_map) # fetch minibatch.
            data16 = cast.eval(mb, as_numpy=False)
            trainer.train_minibatch({criterion.arguments[0]:data16[cast[0]], criterion.arguments[1]:data16[cast[1]]})
            sample_count += mb[label].num_samples                     # count samples processed so far
...
def evaluate(reader, criterion, device=None, minibatch_size=16, max_samples=None):
    ...
    while True:
        ...
        data16 = cast.eval(mb, as_numpy=False)
        metric = evaluator.test_minibatch({criterion.arguments[0]:data16[cast[0]], criterion.arguments[1]:data16[cast[1]]}, device=device)
        samples_evaluated += minibatch_size
        progress_printer.update(0, mb[label].num_samples, metric) # log progress
