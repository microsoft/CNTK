...
def resnet_imagenet(train_data, test_data, mean_data, network_name, epoch_size, num_quantization_bits=32, block_size=None, warm_up=0, 
    ...
    feature32 = C.input_variable((num_channels, image_height, image_width))
    label32 = C.input_variable((num_classes))
    cast = C.combine(C.cast(feature32, dtype=np.float16), C.cast(label32, dtype=np.float16))

    input_map = {
        feature32: train_source.streams.features,
        label32: train_source.streams.labels
    }
