...
def discretized_mix_logistic_loss(x,l):
    ...
    x = ct.reshape(x, (1,)+xs) + ct.constant(value=0., shape=(nr_mix,)+xs) # tf.zeros(xs + [nr_mix]) # here and below: getting the means and adjusting them based on preceding sub-pixels
...
def discretized_mix_logistic_loss_HWC(x,l):
    ...
    log_probs = ct.reshape(ct.reduce_sum(log_probs, axis=2), shape=ls[:-1]+(10,)) + nn.log_prob_from_logits(logit_probs, axis=len(logit_probs)-1)
    losses = nn.log_sum_exp(log_probs, axis=len(log_probs)-1)
    # loss = -ct.reduce_sum(losses)
    loss = ct.reshape(-ct.reduce_sum(ct.reduce_sum(losses,axis=0),axis=1), shape=(1,))
    return loss
