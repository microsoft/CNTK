...
def np_sample_from_discretized_mix_logistic(l,nr_mix=10):
    ...
    means = np.sum(l[:,:,:,:nr_mix]*sel,3)
    log_scales = np.maximum(np.sum(l[:,:,:,nr_mix:2*nr_mix]*sel,3), -7.)
    coeffs = np.sum(np.tanh(l[:,:,:,2*nr_mix:3*nr_mix])*sel,3)
    ...
    # (HWC)
    image = np.concatenate((np.reshape(x0,xs[:-1]+(1,)), np.reshape(x1,xs[:-1]+(1,)), np.reshape(x2,xs[:-1]+(1,))), axis=2)
    return np.ascontiguousarray(np.transpose(image, (2,0,1))) # From HWC to CHW
