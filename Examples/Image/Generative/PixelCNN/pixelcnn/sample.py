...
def np_sample_from_discretized_mix_logistic_2(l, nr_mix=10):
    ...
    # unpack parameters
    logit_probs = l[:,:nr_mix,:,:]
    l = np.reshape(l[:,nr_mix:,:,:], (xs[0], xs[1], nr_mix*3)+xs[2:])

    # sample mixture indicator from softmax
    sel = nn.one_hot(np.argmax(logit_probs - np.log(-np.log(np.random.uniform(1e-5, 1. - 1e-5, logit_probs.shape).astype('f'))), axis=1), depth=nr_mix, axis=1 , dtype=np.float32)
    sel = np.reshape(sel, (xs[0], 1, nr_mix) + xs[2:])

    # select logistic parameters
    means = np.sum(l[:,:,:nr_mix,:,:]*sel,2)
    log_scales = np.maximum(np.sum(l[:,:,nr_mix:2*nr_mix,:,:]*sel,2), -7.)
    coeffs = np.sum(np.tanh(l[:,:,2*nr_mix:3*nr_mix,:,:])*sel,2)
