...
class TreeMap():
    ...
    def tree_map_from_tree_str(tree_str, use_background=True, use_multiply_with_parent=True):
        ...
        :param use_multiply_with_parent: whether or not the outputs each classes conditional propability shall be
        multiplied with its parents propability.
        ...
        return TreeMap(root_node, meta_map, use_background=use_background,
                       use_multiply_with_parent=use_multiply_with_parent)
    ...
    def _create_single_row_view(self):
        ...
        self.as_in_network = completely_flat[
                             1:]  # all nodes behind another, excluding root, including backgrouds as None
    ...
    def tree_softmax(self, model, axis=0, offset=0):
        ...
        def _multiply_with_parents(slices, tree_map):
            ...
            for i in range(1, len(regions)):  # skip region below root since it need not be multiplied
                ...
                if first_node is None: first_node = tree_map.as_in_network[
                    regions[i][0] + 1]  # when using background, the first node is set to none (background)
                ...
                multiplier = cntk.ops.slice(my_output_slices[j], axis=axis, begin_index=offset_in_region,
                                            end_index=offset_in_region + 1,
                                            name="parent_value_from_region_" + str(j) + "_offset_" + str(
                                                offset_in_region))
        ...
        for i in range(len(self.softmax_regions)):
            ...
            region = cntk.ops.slice(model, axis=axis, begin_index=indices[0] + offset, end_index=indices[1] + offset,
                                    name="Region_" + str(i) + "_from_" + str(indices[0] + offset) + "_to_" + str(
                                        indices[1] + offset))
    ...
    def get_train_softmax_vectors(self, to_Set, scale_value, dtype=np.float32):  # to_set: List<Tuple<ClassMap, int>>
        ...
        :param to_Set: list of tuples (cls_map, original_label) of labels to predict. The original_label refers to the
        label in the given ClassMap.
...
class Output_Mapper():
    ...
    def get_prediciton_vector(self, network_output):
        """
        Removes the background predictions of the different softmax-regions despite the root one if use_background is
        true. Thereby mapping outputs to the indicies in the extended classmap.
