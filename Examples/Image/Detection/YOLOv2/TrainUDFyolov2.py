...
class TrainFunction(UserFunction):
        ...
        for sample in range(mb_size):
            ...
            for i in range(len(gtb_array)):
                ...
                for z in range(self.num_anchorboxes):
                    ...
                    # TODO Proposal: use same x & y  (e.g. 0,0) for gtb and ab and make iou dependent only on w&h --> find ab that fits the best and not the one that has the best position
                ...
                if (highest_iou_index is not None):
                    ...
                    predicted_bb = eval_results[sample][vector_index][0:4]
                    ...
                    if actual_iou > array_goal[sample][vector_index][4]:
                        array_goal[sample][vector_index][0] = gtb_array[i][0]
                        array_goal[sample][vector_index][1] = gtb_array[i][1]
                        array_goal[sample][vector_index][2] = gtb_array[i][2]
                        array_goal[sample][vector_index][3] = gtb_array[i][3]
                        array_goal[sample][vector_index][4] = actual_iou

                        # set scale for responsible vector
                        #for z in range(4):
                        #    array_scale[slice][vector_index][z] = self.lambda_coord
                        array_scale[sample][vector_index][0:4] = self.lambda_coord
                        array_scale[sample][vector_index][4] = 1
                        ...
                        for ab in range(self.num_anchorboxes):
                            ...
                            if index != vector_index:
                                array_scale[sample][index][4] = 0 # set obj of non relevant values --> 0
        ...
        if True:
            ...
            for sample in range(mb_size):
                for i in range(gtb_inputs.shape[1]):
                    cls_index = cls_indexs[sample][i]
                    if cls_index < 0: break #first padding box reached!
                    array_goal[ sample,
                                gtb_ymins[sample][i]:gtb_ymaxs[sample][i] + 1,
                                gtb_xmins[sample][i]:gtb_xmaxs[sample][i] + 1,
                                :,
                                cls_indexs[sample][i] ] += 1
                ...
                for i in range(len(gtb_array)):
                    ...
                    for x in range(left_gc_index, right_gc_index + 1):
                        for y in range(top_gc_index, bottom_gc_index + 1):
                            ...
                            for z in range(self.num_anchorboxes):
                                array_goal[sample][curr_vector_offset + z][cls_index] += 1
                                divisors[curr_vector_offset + z] += 1

                    for z in range(len(divisors)):
                        if divisors[z] > 0:
                            for s in range(num_cls):
                                array_scale[sample][z][s]=1
                                array_goal[sample][z][s] /= divisors[z]
    ...
    def create_outputs(self, eval_results, gtb_inputs):
        ...
        scale = np.append(top_array_scale, bottom_array_scale, axis=2)
