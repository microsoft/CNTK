...
from cntk import distributed ## preparation for distributed learning
import PARAMETERS
import numpy as np
import os, sys
...
def train_fast_rcnn(debug_output=False, model_path=model_file, num_quantization_bits=1, distributed_after=1):
    if debug_output:
    ...
    # Instantiate the trainer object for distributed learner
    local_learner = momentum_sgd(frcn_output.parameters, lr_schedule, mm_schedule, l2_regularization_weight=l2_reg_weight)
    learner = distributed.data_parallel_distributed_learner(local_learner, num_quantization_bits=num_quantization_bits, 
