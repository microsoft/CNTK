...
def eval_fast_rcnn_mAP(eval_model, img_map_file=None, roi_map_file=None):
    ...
    data_path = base_path
    data_set = "test"
    minibatch_source = create_mb_source(image_height, image_width, num_channels, num_original_classes, rois_per_image,
                                        data_path, data_set)
    input_map = {
    ...
    for img_i in range(0, num_test_images):
        ...
        if use_gt_as_rois:
            rois = np.zeros(rois.shape, dtype=np.float32)
            num_gts = len(gt_data)

            rois[0:num_gts] = gt_data[:, :4]
            rois[:, [0, 2]] *= 9 / 16
            rois[:, [0, 2]] += 7 / 32
            rois *= output_scale + output_scale
    ...
    bb_img_rois_l = visualize_rois(all_boxes, False)
