...
def conv2d_getter(f, attr):
    ...
    if attr.has_bias:
        ...
        b = None
    return crosstalk.Conv2DArgs(W=W, b=b)
...
def _get_birnn_param(f):
    ...
    return crosstalk.RnnArgs(fw_W=find_func_param(fw, name='W'),
...
def rnn_getter(f, attr):
    ...
    if use_cudnn:
        ...
        return crosstalk.RnnArgs(fw_W=_adjust_lstm_gate_order(fw_Wt.reshape(gates*attr.hidden_dim, -1).transpose()),
        ...
        return crosstalk.RnnArgs(fw_W=parameter_getter(param.fw_W),
...
class CNTKCrosstalk(crosstalk.Crosstalk):
    def __init__(self):
        ...
        super(CNTKCrosstalk, self).register_funcs(crosstalk.Conv2DAttr, setter=conv2d_setter, getter=conv2d_getter)
        super(CNTKCrosstalk, self).register_funcs(crosstalk.RnnAttr, setter=rnn_setter, getter=rnn_getter)
        super(CNTKCrosstalk, self).register_funcs(crosstalk.EmbedAttr, setter=embed_setter, getter=embed_getter)
