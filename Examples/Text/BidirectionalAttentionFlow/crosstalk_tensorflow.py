...
def conv2d_getter(sess):
    def _get(pd, attr):
        ...
        if attr.has_bias:
            ...
            b = None
        return crosstalk.Conv2DArgs(W=W.transpose(3,0,1,2), b=b.reshape(attr.num_filters,1,1,))
    return _get
...
def rnn_getter(sess):
    def _get(scope, attr):
        ...
        return crosstalk.RnnArgs(fw_W=fw_W, fw_H=fw_H, fw_b=fw_b, bw_W=bw_W, bw_H=bw_H, bw_b=bw_b)
    return _get
...
class TensorFlowCrosstalk(crosstalk.Crosstalk):
    def __init__(self):
    ...
    def set_data(self, sess, data):
        ...
        super(TensorFlowCrosstalk, self).register_funcs(crosstalk.Conv2DAttr, setter=conv2d_setter(sess), getter=conv2d_getter(sess))
        super(TensorFlowCrosstalk, self).register_funcs(crosstalk.RnnAttr, setter=rnn_setter(sess), getter=rnn_getter(sess))
        super(TensorFlowCrosstalk, self).register_funcs(crosstalk.EmbedAttr, setter=embed_setter(sess), getter=embed_getter(sess))
