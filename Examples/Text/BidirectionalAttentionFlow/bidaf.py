...
mb_source = C.MinibatchSource(C.CTFDeserializer('squad_small.ctf', C.StreamDefs(
...
# todo load actual glove
glove = C.parameter(shape=(C.InferredDimension, glove_dim), init=C.glorot_uniform())
...
q_spliced_input = C.splice(C.times(qw, glove), C.reshape(charcnn(qchars), convs))
...
# todo we need to reshape because GlobalMaxPooling is retaining a trailing singleton dimension
c_spliced_input = C.splice(C.times(cw, glove), C.reshape(charcnn(cchars), convs))
