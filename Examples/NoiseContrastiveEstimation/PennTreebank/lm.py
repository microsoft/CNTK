...
from cntk.learner import sgd, adam_sgd, learning_rate_schedule, momentum_schedule, momentum_as_time_constant_schedule
from cntk.ops import sampled_times, nce_criterion, cross_entropy_with_softmax, classification_error
...
def train(reader, model, max_epochs):
    ...
    input_words = Input(input_dim,  is_sparse=True)
    output_labels = Input(input_dim,  is_sparse=True)

    #----------------------------------------------
    W = Parameter((hidden_dim,label_dim), init=init_default_or_glorot_uniform, name='W')

    #------------------------------------------------
    
    # apply model to input
    x = model(input_words)
    #x = sigmoid( times(input_words, W1) )
    #z = times(x, W2)
    #z = times_transpose(x, W)
    z = sampled_times(x, W, output_labels)
    ...
    #ce = nce_criterion(output_labels, z)
    #pe = nce_criterion(output_labels, z)
