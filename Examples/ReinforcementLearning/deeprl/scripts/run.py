...
parser.add_argument('--max_steps', type=int, default=1000000, help='Maximum '
                    'steps to train an agent.')
parser.add_argument('--max_episode_steps', type=int, default=0, help='Maximum '
                    'steps per episode. Use environment specific value if 0.')
parser.add_argument('--eval_period', type=int, default=250000, help='Number '
                    'of steps taken between each evaluation.')
parser.add_argument('--eval_steps', type=int, default=125000, help='Number of '
                    'steps taken during each evaluation.')
parser.add_argument('--verbose', action='store_true', help='Output debug info '
                    'if set to True.')
parser.add_argument('--output_dir', type=str, default='out', help='Directory '
                    'where workspace file and model file are saved to. Model '
                    'file will be named as output_dir.model, and workspace '
                    'file will be named as output_dir.wks.')
...
# Create an OpenAI Gym environment, and obtain its state/action information.
...
def evaluate_agent_if_necessary():
    ...
    if agent.step_count >= eval_count * args.eval_period:
        ...
        filename_prefix = args.output_dir + '/' + args.output_dir
...
while True:
    ...
    if agent.step_count >= args.max_steps:
        break
    # Evaluate agent every --eval_period steps.
