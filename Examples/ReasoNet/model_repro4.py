...
def attention_module(context_memory, query_memory, entity_memory, init_status, hidden_dim, max_steps = 5, init = init_default_or_glorot_uniform):
  ...
  for step in range(max_steps):
    ...
    prob_norm = element_divide(output[step*2+1], sequence.reduce_sum(output[step*2]))
    output[step*2] = ops.element_times(output[step*2], sequence.broadcast_as(prob_norm, output[step*2]))
