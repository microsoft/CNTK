...
def project_cosine_sim(dim, init = init_default_or_glorot_uniform, name=''):
  cell_shape = (dim, dim)
  Wi = Parameter(cell_shape, init = init, name='Wi')
  Wm = Parameter(cell_shape, init = init, name='Wm')
  status = placeholder_variable(shape=(dim,), name='status')
  memory = placeholder_variable(shape=(dim,), name='memory')
...
def termination_gate(dim, init = init_default_or_glorot_uniform, name=''):
...
def attention_module(context_memory, query_memory, entity_memory, init_status, hidden_dim, max_steps = 5, init = init_default_or_glorot_uniform):
  ...
  sum_prob = None
  context_cos_sim = project_cosine_sim(hidden_dim, name='context_attention')
  query_cos_sim = project_cosine_sim(hidden_dim, name='query_attention')
  ans_cos_sim = project_cosine_sim(hidden_dim, name='candidate_attention')
  stop_gate = termination_gate(dim=hidden_dim, name='terminate_prob')
  prev_stop = 0
...
def loss(model):
  ...
  entity_condition = model.entity_condition
  entities_all = model.entities_all
  answers = sequence.gather(model.outputs[-1], entity_condition, name='Final_Ans')
  labels = sequence.gather(labels_raw, entity_condition, name='EntityLabels')
  entity_ids = model.entity_ids
  entity_dim = model.entity_dim
  entities_all = model.entities_all
