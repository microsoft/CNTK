...
import numpy as np
import ReasoNet.model_repro5 as rsn
...
def testReasoNetTrain(data, epoch_size, max_epochs=1, vocab_dim=101000, entity_dim=101, hidden_dim=300, embedding_dim=100, max_rl_iter =5, embedding_path=None, vocab_path=None, eval_path=None, eval_size=None):
  train_data = rsn.create_reader(data, vocab_dim, entity_dim, True, rand_size=epoch_size)
  ...
  model = rsn.create_model(vocab_dim, entity_dim, hidden_dim, embedding_init=embedding_init, embedding_dim=embedding_dim, max_rl_iter=max_rl_iter)
