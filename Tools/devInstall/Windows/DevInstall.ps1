...
Function main
{
    try { if (-not (DisplayStart -NoConfirm $NoConfirm)) {
        ...
        $operation += OpProtoBuf352VS17 -cache $localCache -targetFolder $localDir -repoDirectory $CloneDirectory
        #$operation += OpProtoBuf352VS17Prebuild -cache $localCache -targetFolder $localDir
