...
#pragma region Constructors and Destructor
    ...
    void GPUSparseMatrix<ElemType>::ZeroInit(const MatrixFormat matrixFormat, const DEVICEID_TYPE computeDevice)
    {
        ...
        m_blockSize = 0;
        m_blockId2col = nullptr;
        m_col2blockId = nullptr;
    ...
    GPUSparseMatrix<ElemType>::GPUSparseMatrix(GPUSparseMatrix<ElemType>&& moveFrom)
    {
        ...
        m_blockId2col = moveFrom.m_blockId2col;
        m_col2blockId = moveFrom.m_col2blockId;
    ...
    void GPUSparseMatrix<ElemType>::Clear()
    {
        ...
        if(m_blockId2col != nullptr) 
            CUDACALL(cudaFree(m_blockId2col));
        if (m_col2blockId!=nullptr)
            CUDACALL(cudaFree(m_col2blockId));
    ...
    {               
        ...
        size_t oldCompIndexSize = max(m_numRows, m_numCols) + 1;
            ...
            if (reallocate)
            {
                ...
                if (m_pArray != nullptr)
                {
                    if (keepExistingValues)
                    {
                        ...
                        CUDACALL(cudaMemcpy(pArray + numNZElemToReserve, MajorIndexLocation(), MajorIndexSize(), cudaMemcpyDeviceToDevice));
                        CUDACALL(cudaMemcpy((GPUSPARSE_INDEX_TYPE*)(pArray + numNZElemToReserve) + numNZElemToReserve, SecondaryIndexLocation(), SecondaryIndexSize(), cudaMemcpyDeviceToDevice));
                    }
        ...
        else if (matrixFormat == MatrixFormat::matrixFormatSparseBlockCol || matrixFormat == MatrixFormat::matrixFormatSparseBlockRow)
        {
            ...
            {
                ElemType * pArray = nullptr;
                CUDACALL(cudaMalloc((void **)&pArray, sizeof(ElemType)*numNZElemToReserve));

                if (m_pArray != nullptr)
                {
                    if (keepExistingValues)
                    {
                        if (m_nz > numNZElemToReserve)
                            LogicError("Resize: To keep values m_nz should <= numNZElemToReserve.");
            ...
            if (oldCompIndexSize < newCompIndexSize || (oldCompIndexSize > newCompIndexSize && !growOnly))
            {
                if (m_blockId2col != nullptr)
                    CUDACALL(cudaFree(m_blockId2col));
                if (m_col2blockId != nullptr)
                ...
                CUDACALL(cudaMalloc((void **)&m_blockId2col, sizeof(GPUSPARSE_INDEX_TYPE)*newCompIndexSize));
                CUDACALL(cudaMalloc((void **)&m_col2blockId, sizeof(GPUSPARSE_INDEX_TYPE)*newCompIndexSize));
...
#pragma region Static BLAS Functions
        ...
        const GPUSparseMatrix<ElemType>& rhs, const bool transposeB, GPUSparseMatrix<ElemType>& c)
    {
        ...
        {   
            ...
            if (n * 4 < threadsPerBlock * rhs.m_nz)
            {
                c.Resize(m, n, 1, true, false); //reserve memory for m_blockId2col and m_col2blockId

                size_t *blockSize;
                CUDACALL(cudaMalloc((void **)&blockSize, sizeof(size_t)));
                CUDACALL(cudaMemset(blockSize, 0, sizeof(size_t)));

                CUDACALL(cudaMemset(c.m_blockId2col, 0, sizeof(GPUSPARSE_INDEX_TYPE)*(n)));
                //CUDACALL(cudaMemset(c.m_col2blockId, 0, sizeof(GPUSPARSE_INDEX_TYPE)*(n)));
            ...
            else
            {
                ...
                c.m_nz = nnz;
                CUDACALL(cudaMemset(c.NzValues(), 0, sizeof(ElemType)*(c.m_nz)));
                CUDACALL(cudaMemset(c.m_blockId2col, 0, sizeof(GPUSPARSE_INDEX_TYPE)*(c.m_blockSize)));
        ...
        const GPUMatrix<ElemType>& idx2cls, GPUSparseMatrix<ElemType>& grd)
    {
        ...
        CUDACALL(cudaMemset(grd.m_blockId2col, 0, sizeof(GPUSPARSE_INDEX_TYPE)*(grd.m_blockSize)));
    ...
    size_t GPUSparseMatrix<ElemType>::ElemCountFromBufferSize(const size_t numRows, const size_t numCols, const MatrixFormat format, const size_t totalBufferSize) const
    {
        ...
        {
            elemSizeAllocated = (totalBufferSize - sizeof(GPUSPARSE_INDEX_TYPE)*SecondaryIndexCount(numRows, numCols, 0, format)) / (sizeof(GPUSPARSE_INDEX_TYPE)+sizeof(ElemType));
        }
