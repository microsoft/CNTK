...
#define ValOpWithRed(opCode, shape) (pair<function<NDArrayViewPtr(const vector<NDArrayViewPtr>&)>, const char*>(ValExpr(NDArrayView::NumericOperation(argValues, 1.0, L#opCode, make_shared<NDArrayView>(dataType, NDShape(shape), device), 0, L"Sum")), #opCode "|Reduce"))
    ...
    vector<TensorViewTest> tests =
    {
        ...
        // affine variant
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], false, argValues[1], false, 1.0, 1) ), "Affine_shared_W" }, VarExpr(CNTK::Affine         (args[0], args[1], args[2]   )),{ { 13, 42 },{ 42, 9    },{ 13 } } },
        ...
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], false, argValues[1], false, 1.0, 1) + argValues[2]), "Affine"          }, VarExpr(CNTK::Affine         (args[0], args[1], args[2]   )),{ { 13, 42 },{ 42, 9    },{ 13 } } },
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], false, argValues[1], false, 1.0, 1) + argValues[2]), "Affine"          }, VarExpr(CNTK::Affine         (args[0], args[1], args[2]   )),{ { 13, 42 },{ 42       },{ 13 } } },
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], false, argValues[1], false, 1.0, 1) + argValues[2]), "Affine"          }, VarExpr(CNTK::Affine         (args[0], args[1], args[2], 0)),{ {     42 },{ 42       },{    } } }, // should get batched
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], false, argValues[1], false, 1.0, 1) + argValues[2]), "Affine"          }, VarExpr(CNTK::Affine         (args[0], args[1], args[2]   )),{ { 13, 42 },{ 42, 9, 5 },{ 13 } } },
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], true,  argValues[1], false, 1.0, 1) + argValues[2]), "TransposeAffine" }, VarExpr(CNTK::TransposeAffine(args[0], args[1], args[2]   )),{ { 42, 13 },{ 42, 9    },{ 13 } } },
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], true,  argValues[1], false, 1.0, 1) + argValues[2]), "TransposeAffine" }, VarExpr(CNTK::TransposeAffine(args[0], args[1], args[2]   )),{ { 42, 13 },{ 42       },{ 13 } } },
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], true,  argValues[1], false, 1.0, 1) + argValues[2]), "TransposeAffine" }, VarExpr(CNTK::TransposeAffine(args[0], args[1], args[2]   )),{ { 42, 13 },{ 42, 9, 3 },{ 13 } } },
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], true,  argValues[1], false, 1.0, 1) + argValues[2]), "Transpose|Affine()" }, VarExpr(CNTK::Affine(CNTK::Transpose(args[0])                      , args[1], args[2])),{ { 42, 13 },{ 42, 9, 3 },{ 13 } } }, // Transpose optimization, from axes
        { { ValExpr(NDArrayView::MatrixProduct(false, argValues[0], true,  argValues[1], false, 1.0, 1) + argValues[2]), "Transpose|Affine{}" }, VarExpr(CNTK::Affine(CNTK::Transpose(args[0], AxisVector({ 1, 0 })), args[1], args[2])),{ { 42, 13 },{ 42, 9, 3 },{ 13 } } }, // Transpose optimization, from permutation
