...
#define RedOp(redOpCode, shape, denom) (pair<function<NDArrayViewPtr(const vector<NDArrayViewPtr>&)>, const char*>([=](const vector<NDArrayViewPtr>& argValues){ return NDArrayView::NumericOperation(argValues, 1.0/denom, opCopy, make_shared<NDArrayView>(dataType, NDShape(shape), device), 0, op##redOpCode); }, "Reduce" #redOpCode))
    vector<TensorViewTest> tests =
    {
        ...
        //{ { [&](const vector<NDArrayViewPtr>& argValues) { return argValues[0]->SlicedTensorView({ 2, 0 }, {  3, 42 }); }, "Slice" }, [&](const vector<Variable>& args) { return CNTK::Slice(args[0], { Axis(0) }, { 2 }, { 2+3 }); },{ { 13, 42 } } }, // non-contiguous slice
        ...
        //{ { [&](const vector<NDArrayViewPtr>& argValues) { return argValues[0]->SlicedTensorView({ 0, 1 }, { 13,  4 }); }, "Slice" }, [&](const vector<Variable>& args) { return CNTK::Slice(args[0], { Axis(1) }, { 1 }, { 1+4 }); },{ { 13, 42 } } }, // same but testing SlicedTensorView() on the reference path
        ...
        //{ { [&](const vector<NDArrayViewPtr>& argValues) { return argValues[0]->SlicedTensorView({ 0, 3 }, {     13 }); }, "Index" }, [&](const vector<Variable>& args) { return CNTK::Index(args[0], 3); },{ { 13, 42 } } }, // index of rank > 1; also 6esting SlicedTensorView()
        // matrix product
