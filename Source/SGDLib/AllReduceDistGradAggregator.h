...
namespace Microsoft { namespace MSR { namespace CNTK {
    ...
    public:
        AllReduceDistGradAggregator(MPIWrapper *mpi, bool zeroThresholdFor1Bit, bool useQuantizationForSelfStripe, bool useAsyncAggregation, int traceLevel, int syncStatsTrace) :
            IDistGradAggregator<ElemType>(mpi), m_zeroThresholdFor1Bit(zeroThresholdFor1Bit),
            m_useQuantizationForSelfStripe(useQuantizationForSelfStripe), m_traceLevel(traceLevel), m_currentEpochNumber(-1),
            m_useAsyncAggregation(useAsyncAggregation), m_bufferedGradHeader(nullptr), m_syncStatsTrace(syncStatsTrace), m_iterationCount(0)
        {
        }
        ...
        bool ResetCurrentEpoch(const std::vector<Matrix<ElemType>*>& gradients, int numEvalNode, int nBits, int epochNumber)
        {
            ...
            if (m_currentEpochNumber == -1)
            {
                ...
                for (size_t i = 0; i < gradients.size(); i++)
                {
                    ...
                    m_preAggGradQuantizers.push_back(std::unique_ptr<MatrixQuantizer<ElemType>>(MatrixQuantizer<ElemType>::CreateMatrixQuantizer(nRow, nCol, deviceId, m_useAsyncAggregation)));
                    m_gradQuantized.push_back(std::unique_ptr<QuantizedMatrix<ElemType>>(new QuantizedMatrix<ElemType>(nRow, nCol, nBits, CPUDEVICE, m_allocator.get())));
                    ...
                    {
                        currAggGradQuantizer = MatrixQuantizer<ElemType>::CreateMatrixQuantizer(nRow, stripe.m_numCols, deviceId, m_useAsyncAggregation);
                        for (size_t j = 0; j < NumProc() - 1; ++j)
                        {
                            currRecvGradStripesQuantized.push_back(std::unique_ptr<QuantizedMatrix<ElemType>>(new QuantizedMatrix<ElemType>(nRow, stripe.m_numCols, nBits, CPUDEVICE, m_allocator.get())));
                        }
                    }
        ...
        bool AggregateGradients(const std::vector<Matrix<ElemType>*>& gradients, DistGradHeader *headerCPU, int nBits, int epochNumber) override
        {
            bool isNewEpoch = ResetCurrentEpoch(gradients, headerCPU->numEvalNode, nBits, epochNumber);
        ...
        void AggregateGradientsImpl(const std::vector<Matrix<ElemType>*>& gradients, DistGradHeader *headerCPU, bool showSyncPerfStats)
        {
            ...
            while (numActualReceives < numReceivesExpected)
            {
                ...
                // is used for both and they are impicitly sequenced
