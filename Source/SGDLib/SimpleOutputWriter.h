...
public:
    ...
    void forward_decode(Sequence oneSeq, StreamMinibatchInputs decodeinputMatrices, DEVICEID_TYPE deviceID, std::vector<ComputationNodeBasePtr> decodeOutputNodes,
                        std::vector<ComputationNodeBasePtr> decodeinputNodes, std::vector<ComputationNodeBasePtr> biasNodes, std::vector<ComputationNodeBasePtr> biasOutputNodes, Matrix<ElemType>& bias, size_t vocabSize, size_t plength)
    {
        ...
        if (plength != oneSeq.processlength)
        {
            ...
            lminput->second.pMBLayout->AddSequence(0, 0, 0, plength);

            //add bias value
            (&dynamic_pointer_cast<ComputationNode<ElemType>>(biasNodes[0])->Value())->SetValue(bias);            
            auto PlusMBlayout = biasNodes[0]->GetMBLayout();
            PlusMBlayout->Init(1, bias.GetNumCols());
            PlusMBlayout->AddSequence(0, 0, 0, bias.GetNumCols());
            ComputationNetwork::BumpEvalTimeStamp(decodeinputNodes);
            ComputationNetwork::BumpEvalTimeStamp(biasNodes);
            m_net->ForwardPropFromTo(biasNodes, biasOutputNodes);
            /*DataReaderHelpers::NotifyChangedNodes<ElemType>(m_net, decodeinputMatrices);
            ComputationNodeBasePtr embeddingNode = m_net->GetNodeFromName(L"embedding_s");
            std::vector<ComputationNodeBasePtr> embeddingNodes;
            embeddingNodes.push_back(embeddingNode);
            m_net->ForwardPropFromTo(decodeinputNodes, embeddingNodes);
            std::vector<ComputationNodeBasePtr> twoinputs = embeddingNodes;
            twoinputs.insert(twoinputs.end(), biasOutputNodes.begin(), biasOutputNodes.end());
            ComputationNetwork::BumpEvalTimeStamp(embeddingNodes);
            ComputationNetwork::BumpEvalTimeStamp(biasOutputNodes);*/
            //m_net->ForwardPropFromTo(twoinputs, decodeOutputNodes);
            //(&dynamic_pointer_cast<ComputationNode<ElemType>>(biasNodes[0])->Value())->Print("bias");
            m_net->ForwardProp(decodeOutputNodes[0]);
    ...
    void forwardmerged(Sequence a, size_t t, Matrix<ElemType>& sumofENandDE, Matrix<ElemType>& encodeOutput, Matrix<ElemType>& decodeOutput, ComputationNodeBasePtr PlusNode, ComputationNodeBasePtr PlusTransNode, std::vector<ComputationNodeBasePtr> Plusnodes, std::vector<ComputationNodeBasePtr> Plustransnodes)
    {
        ...
        auto PlusMBlayout = PlusNode->GetMBLayout();
        PlusMBlayout->Init(1, 1);
        PlusMBlayout->AddSequence(0, 0, 0, 1);
        m_net->ForwardPropFromTo(Plusnodes, Plustransnodes);
        decodeOutput.SetValue(*(&dynamic_pointer_cast<ComputationNode<ElemType>>(PlusTransNode)->Value()));
        //decodeOutput.VectorMax(maxIdx, maxVal, true);
                          ...
                          size_t numOutputSamples = requestDataSize, bool doWriterUnitTest = false, size_t beamSize = 10, size_t expandBeam = 20, string dictfile = L"", ElemType thresh = 0.68)
    {
        ...
        Matrix<ElemType> decodeOutput(deviceid);
        Matrix<ElemType> greedyOutput(deviceid), greedyOutputMax(deviceid);
        Matrix<ElemType> sumofENandDE(deviceid), maxIdx(deviceid), maxVal(deviceid), bias(deviceid), biasOutput(deviceid);
        Matrix<ElemType> lmin(deviceid);
        ...
        //get bias output
        std::wstring biasOutputNodeName = L"bias_embedding";
        std::wstring biasInputNodeName = L"bias";
        ComputationNodeBasePtr biasNode = m_net->GetNodeFromName(biasInputNodeName);
        ComputationNodeBasePtr biasOutputNode = m_net->GetNodeFromName(biasOutputNodeName);
        std::vector<ComputationNodeBasePtr> biasNodes, biasOutputNodes;
        std::list<ComputationNodeBasePtr> biasNodelist;
        biasNodelist.push_back(biasNode);
        biasNodes.push_back(biasNode);
        biasOutputNodes.push_back(biasOutputNode);
        //m_net->UpdateEvalOrder(biasOutputNode, biasNodelist);
        bias.Resize(vocabSize, keywords[0].size() - 1);
        bias.SetValue(0.0);
        for (size_t n = 1; n < keywords[0].size(); n++)
        {
            bias(keywords[0][n], n - 1) = 1.0;
        }
        /*(&dynamic_pointer_cast<ComputationNode<ElemType>>(biasNode)->Value())->SetValue(bias);
        ComputationNetwork::BumpEvalTimeStamp(biasNodes);
        
        auto PlusMBlayout = biasNode->GetMBLayout();
        PlusMBlayout->Init(1, 1);
        PlusMBlayout->AddSequence(NEW_SEQUENCE_ID, 0, 0, keywords[0].size() - 1);*/
        //m_net->ForwardPropFromTo(biasNodes, biasOutputNodes);
        //biasOutput.SetValue(*(&dynamic_pointer_cast<ComputationNode<ElemType>>(biasOutputNode)->Value()));
        ...
        forward_decode(oneSeq, decodeinputMatrices, deviceid, decodeOutputNodes, decodeinputNodes, biasNodes, biasOutputNodes, bias, vocabSize, oneSeq.labelseq.size());
        preComputeSequence.push_back(oneSeq);

        Sequence anotherSeq = newSeq(oneSeq);
        extendSeq(anotherSeq, keywords[0][1], 0.0);
        forward_decode(anotherSeq, decodeinputMatrices, deviceid, decodeOutputNodes, decodeinputNodes, biasNodes, biasOutputNodes, bias, vocabSize, anotherSeq.labelseq.size());
        ...
        for (size_t ikey = 0; ikey < keywords.size(); ikey++)
        {
            for (size_t labelId = 2; labelId <= keywords[ikey].size(); labelId++)
            {
                ...
                if (it == preComputeSequence.end())
                {
                    ...
                    forward_decode(tmpseq, decodeinputMatrices, deviceid, decodeOutputNodes, decodeinputNodes, biasNodes, biasOutputNodes, bias, vocabSize, tmpseq.labelseq.size());
            ...
            for (size_t t = 0; t < encodeOutput.GetNumCols(); t++)
            {
                ...
                while (true)
                {
                    ...
                    if (itseq != preComputeSequence.end())
                    {
                        ...
                        forward_decode(tempSeq, decodeinputMatrices, deviceid, decodeOutputNodes, decodeinputNodes, biasNodes, biasOutputNodes, bias, vocabSize, tempSeq.labelseq.size());
