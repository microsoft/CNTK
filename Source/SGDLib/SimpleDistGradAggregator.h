...
namespace Microsoft { namespace MSR { namespace CNTK {
    ...
    public:
        SimpleDistGradAggregator(MPIWrapper *mpi, bool useAsyncAggregation, int syncStatsTrace) 
            : IDistGradAggregator<ElemType>(mpi), m_useAsyncAggregation(useAsyncAggregation), m_currentEpochNumber(-1),
            m_bufferedGradHeader(nullptr), m_syncStatsTrace(syncStatsTrace), m_iterationCount(0)
        {
        }
