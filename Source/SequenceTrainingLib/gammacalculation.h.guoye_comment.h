...
public:
    ...
    void twodimForwardBackward(Microsoft::MSR::CNTK::Matrix<ElemType>& totalScore,
        ...
        const Microsoft::MSR::CNTK::Matrix<ElemType>& maxIndexes,
        Microsoft::MSR::CNTK::Matrix<ElemType>& m_derivative, //guoye: m_derivative is not used in the function
        const std::shared_ptr<Microsoft::MSR::CNTK::MBLayout> pMBLayout,
        const std::shared_ptr<Microsoft::MSR::CNTK::MBLayout> phoneMBLayout,
        size_t blankTokenId)

    {
        const auto numParallelSequences = pMBLayout->GetNumParallelSequences(); // guoye: this should be the #channels?
        const auto numPhoneParallelSequences = phoneMBLayout->GetNumParallelSequences(); // guoye: same here?
        const auto numSequences = pMBLayout->GetNumSequences();  // guoye: this should be the #utterances?
        //assert(numParallelSequences==phoneMBLayout->GetNumParallelSequences());
        assert(numSequences==phoneMBLayout->GetNumSequences()); // guoye: same here?
        const size_t numRows = F.GetNumRows(); // guoye: F includes the padding frames: i.e., when the channel is not fulfilled by one utterance.
        const size_t numCols = F.GetNumCols(); // guoye: yes, number of frames. F can contain more than one utterances.
        const size_t numPhoneCols = G.GetNumCols(); // guoye: number of phones (or word piece)?

		// guoye: what are the below two for?
        size_t maxFrameNum = numCols / numParallelSequences; //guoye: the channel length, it should be at least as large as the maximum utterance length
        ...
        for (size_t s = 0; s < numSequences; s++)
        {
            ...
            totalcol += uttFrameNum[s] * uttPhoneNum[s];   // guoye: for uttPhoneNum, is the begining blank added? It is added at the reader when reading the input, when blankInFront=true
        }
		...
		// before the call of AssignRNNTScore, mergedinput  contains the output probabity. However, after the call of AssignRNNTScore, the mergedinput contains the gradient
        ...
        //guoye: totalscore is the sum of scores of all utterances in the minibatch. Get00Element means gets the 0,0 element of the matrix
