...
// TODO: move to actual text files to be included
    ...
    wstring computationNodes =  // TODO: use actual TypeName() here? would first need to make it a wide string; we should also extract those two methods into the base macro
        ...
        L"Shift(input, fromOffsetDims, boundaryValue, dim=-1, offsetRangeDims=1, multiOffsetDim=0) = new ComputationNode [ operation = 'Shift' ; inputs = (input : boundaryValue) ; fromOffset = new TensorShape [ dims = fromOffsetDims ] ; offsetRange = new TensorShape [ dims = offsetRangeDims ] /*plus the function args*/ ]\n"
