...
bool TextBlockRandomizer::TimelineIsValidForRandomization(const SequenceDescriptions& timeline) const
{
...
void TextBlockRandomizer::RandomizeChunks()
{
    ...
    for (chunkId = 0, samplePosition = 0, sequencePosition = 0; chunkId < m_numChunks; chunkId++)
    {
...
bool TextBlockRandomizer::IsValidForPosition(size_t targetPosition, const SequenceDescription& seqDesc) const
{
...
void TextBlockRandomizer::Randomize()
{
    ...
    foreach_index (i, m_randomTimeline)
    {
        ...
        if (!IsValidForPosition(i, m_randomTimeline[i]))
            LogicError("TextBlockRandomizer::Randomize: randomization logic mangled!");
    }
}

void TextBlockRandomizer::RandomizeIfNewSweepIsEntered()
{
...
void TextBlockRandomizer::RandomizeForGlobalSamplePosition(const size_t samplePosition)
{
...
TextBlockRandomizer::TextBlockRandomizer(int verbosity,
...
void TextBlockRandomizer::Initialize(TransformerPtr next, const ConfigParameters& readerConfig)
{
...
void TextBlockRandomizer::StartEpoch(const EpochConfiguration& config)
{
...
bool TextBlockRandomizer::GetNextSequenceDescriptions(size_t sampleCount, SequenceDescriptions& sequenceDescriptions)
{
...
Sequences TextBlockRandomizer::GetNextSequences(size_t sampleCount)
{
