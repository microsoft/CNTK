...
{
public:
    explicit TransformBase(const Microsoft::MSR::CNTK::ConfigParameters& config)
    {
        m_seed = config(L"seed", 0u);
        std::wstring precision = config(L"precision", L"float");
        if (Microsoft::MSR::CNTK::AreEqualIgnoreCase(precision, L"float"))
            m_precision = DataType::Float;
        else if (Microsoft::MSR::CNTK::AreEqualIgnoreCase(precision, L"double"))
