...
void SequencePacker::PackSparseSample(char* destination, SequenceDataPtr sequence, size_t sample, size_t elementSize, size_t sampleSize)
{
    ...
    size_t nonZeroCount = s->m_nnzCounts[sample];
    ...
    {
        auto rowIndex = s->m_indices[nonZeroIndex];
        size_t elementOffset = rowIndex * elementSize;
        assert(elementOffset < sampleSize);
        auto source = (const char*)(s->m_data) + nonZeroIndex * elementSize;
        memcpy(destination + elementOffset, source, elementSize);
    }
}

// Packs a dense sample as dense.
void SequencePacker::PackDenseSample(char* destination, SequenceDataPtr sequence, size_t sample, size_t /*elementSize*/, size_t sampleSize)
{
