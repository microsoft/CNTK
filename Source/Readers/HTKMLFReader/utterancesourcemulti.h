...
class minibatchutterancesourcemulti : public minibatchsource
{
    ...
    struct utterancechunkdata // data for a chunk of utterances
    {
        ...
        void requiredata(std::string &featkind, size_t &featdim, unsigned int &sampperiod, const latticesource &latticesource, size_t featdiminfile, int verbosity = 0) const
        {
            if (numutterances() == 0)
            ...
            try // this function supports retrying since we read from the unrealible network, i.e. do not return in a broken state
            {
                ...
                foreach_index (i, utteranceset)
                {
                    ...
                    reader.read(utteranceset[i].parsedpath, (const std::string &)featkind, sampperiod, uttframes, featdiminfile, utteranceset[i].needsExpansion);  // note: file info here used for checkuing only
...
public:
    ...
    minibatchutterancesourcemulti(bool useMersenneTwister, const std::vector<std::vector<std::wstring>> &infiles, const std::vector<std::map<std::wstring, std::vector<msra::asr::htkmlfentry>>> &labels,
                                  ...
                                  const latticesource &lattices, const std::map<std::wstring, msra::lattices::lattice::htkmlfwordsequence> &allwordtranscripts, const bool framemode, std::vector<bool> expandToUtt, std::vector<size_t> featdiminfile, const size_t maxUtteranceLength, const bool truncated)
                                  ...
                                  m_frameRandomizer(randomizedchunks, useMersenneTwister), expandToUtt(expandToUtt), m_useMersenneTwister(useMersenneTwister), featdimsinfile(featdiminfile), maxUtteranceLength(maxUtteranceLength), truncated(truncated)
