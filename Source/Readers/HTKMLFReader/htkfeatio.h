...
public:
    ...
    void getinfo(const parsedpath& ppath, string& featkind2, size_t& featdim2, unsigned int& featperiod2, size_t specifieddim)
    {
    ...
    void read(const parsedpath& ppath, const string& kindstr, const unsigned int period, MATRIX& feat, size_t specifieddim, bool needsExpansion=false)
    {
        ...
        size_t numframes2 = open(ppath, specifieddim);
        if (needsExpansion)
        {
            if (numframes2 != 1)
