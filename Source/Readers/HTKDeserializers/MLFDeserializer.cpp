...
protected:
          ...
          m_deserializer(deserializer)
    {
        ...
        auto f = OpenOrDie(fileName, L"rbS");
        ...
        auto chunkOffset = descriptor.StartOffset();

        // Seek and read chunk into memory.
        if (!TrySeek(f, chunkOffset, SEEK_SET))
            RuntimeError("Error seeking to position '%" PRId64 "' in the input file '%ls': %s",
                         chunkOffset, fileName.c_str(), strerror(errno));

        ReadOrDie(m_buffer.data(), sizeInBytes, 1, f);
