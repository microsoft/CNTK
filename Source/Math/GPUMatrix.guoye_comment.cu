                                                          ...
                                                          const bool isColWise)
{
    ...
    if (isColWise)
    {
        ...
        matrixPhoneSeq.Resize(maxPhoneNum, uttNum); // guoye: #rows is he maxphonenumber, #col is the number of utterances.
            _convertPhoneSeq<<<block_tail_c, thread_tail, 0, t_stream>>>(phoneSeq.Data(), matrixPhoneSeq.Data(), gpuPhoneNum, gpuBeginPhone, gpuPhoneToChanInd, uttNum, numPhoneParallelSequences, maxPhoneNum); // guoye: phoneSeq is in CNTK matrix layout
			
        int blocksPerGrid = (int) ceil(1.0 * uttNum / GridDim::maxThreadsPerBlock);
        size_t maxTU = maxFrameNum + maxPhoneNum - 1; // guoye: why needs a minus one here? I guess it is 0-index. Also, the phonenumber include the blank. In the paper Fig1, frameNum is 4, the phone number is 4 as well. maxTU = 7.  Since both T and U are now shift to 0 based, the exit node will then be Alpha(3,3),  In the below loop, tu < 7, thus, the maximum tu is 6, matching what you see.
        ...
        for (size_t tu = 0; tu < maxTU; tu++)
        {
            ...
            _assignRNNTAlphaScore2<<<block_tail3, thread_tail, 0, t_stream>>>(prob.Data(), alpha.Data(), matrixPhoneSeq.Data(),
                                                                              gpuFrameNum, gpuPhoneNum, gpuUttBeginForMergedinput, tu,
       ...
       //beta.Print("beta");
		...
		// guoye: Data() stores the prob of (t, u, k)
        for (int s = 0; s < uttNum; s++)
        {
