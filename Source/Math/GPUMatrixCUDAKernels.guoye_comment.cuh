    ...
    const size_t uttNum)
{
    ...
    if (uttId < uttNum && t <= tu)
    {
        ...
        if (t < frameNum && u < phoneNum)
        {
            ...
            // Actual current phone label
            size_t phoneId = (size_t)(phoneSeq[labelid]); //phone ID of u  // guoye: phoneSeq row# is maxPhoneNum, column# is #utterances. maxPhoneNum is the phone channel length
            ...
            size_t tuID = uttBeginForMerge[uttId] + t * phoneNum + u; //tuID for (t,u) //guoye: alphaScore is not in CNTK minibatch layout. Rather each uttrerance in lattice layout of the RNNT paper: i.e., the x-axis is the #frames, and y-axi i #phones in this utterance (phone sequence).
            ...
            probId_1 = tuID_1 * totalPhoneNum + phoneId; //ID for p(y(u)|t,u-1) // guoye: prob is also not arranged in CNTK minibatch format, it follows the same format as alphaScore, except that each scalar value is represented as a vector of totalPhoneNum, i.e., the output dimension of RNNT. Rui makes the minibatch format into this memory efficient form when adding the output of encoder and decoder in Node PlusBroadcast.
            ...
            if (t == 0 && u == 0)
            {
                alphaScore[tuID] = 0.0; // guoye: log (1) = 0
            }
            else if (t == 0)
            {
...
__global__ void _assignRNNTAlphaScore(
    ...
    const size_t u,
    const size_t maxPhoneNum,   // Maximum length of utterance in this MB
    const size_t totalPhoneNum, // Total number of phones, the output dimension of neural network
    const size_t blankTokenId,
    const size_t uttNum)
{
// guoye: maxphoneNum is the maximum number of phones in an utterance, which is the phone minibatch channel length. totalPhoneNum is the output of the neural network
// guoye: prob: the row dim is the output of the neural network, and the column dim is the sum of all utterances of each one with total_phone_number * total_frame_number.
// guoye: the alphaScore will follow the CNTK  minibatch layout to get the time index (i.e., the number of columns), and the number of rows are the phone channel length, i.e., the maximum number of phones in a channel
// guoye: phoneseq: the row dim is the max phone number of the utterance (i.e., phone channel length), the column dim is the number of utterances
...
// guoye: this computes the gradient before softmax, to store in matrix us
template <class ElemType>
                                   ...
                                   const size_t uttId)
{
    ...
    if (k < totalPhoneNum && t < FrameNum) //guoye: totalPhoneNum is the output layer dimension, FrameNum and PhoneNum are the lengths of utterance frame and phone
    {
        ...
        for (int u = 0; u < PhoneNum; u++)
        {
            ...
            alphaId = tuID;
            ktuID = tuID * totalPhoneNum + k; //ID for P(k|t,u)
            z = from[tuID]; // guoye: real prob, not in log domain now
            ...
            if (u < PhoneNum - 1) //k == y(u+1)
            {
                ...
                if (k == phoneId)
                {
                    ...
                    if (x < LZERO)
                        ...
                        y = exp_((comp_t) x);
                    z -= y; // guoye; why we need to minus y
                }
            }
            //printf("t:%d k:%d u%d step2\n", (int) t, (int) k, int(u));
            ...
            y = z * exp_((comp_t) us[ktuID]); // guoye: current us[ktuID] stores the log pro of P(k|t,u). 
            us[ktuID] = y; // guoye; now, us[ktuID] stores the graident of logit of h(k,t,u)
        }
        //printf("t:%d k:%d step4\n", (int) t, (int) k);
    }
}
