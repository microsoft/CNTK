...
//TODO: Can this be vectorized? I'm not sure whether that would be useful or not.
//Since you're jumping around by column in at least one of the matrices, you're
//probably spending all of your time waiting on elements from main memory.
//Perhaps multithreading would help though.
template<class ScalarT> void TransposeMulti(ScalarT* transposeMe, ScalarT* transposed, int origRows, int origCols)
{
	TransposeArgs<ScalarT> ta;
	ta.origCols = origCols;
	ta.origRows = origRows;
	ta.transposeMe = transposeMe;
	ta.transposed = transposed;
	for (int r = 0; r < origRows; ++r)
	{
		ta.r = r;

		transPool.QueueAndWake(ta);
		//for (int c = 0; c < origCols; ++c)
		//{
		//new c,r = old r,c
		//int oldOffset = RowColToOffset(r, c, origCols);
