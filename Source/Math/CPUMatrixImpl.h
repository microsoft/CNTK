...
template <class ElemType>
CPUMatrix<ElemType>& CPUMatrix<ElemType>::MatrixTimeReduction(CPUMatrix<ElemType>& in1, CPUMatrix<ElemType>& uttInfo, const size_t factor, const size_t numParallelSequences, bool revert)
{
    ...
    for (int s = 0; s < numSequences; s++) //loop for every utt
    {
        ...
        for (int k = 0; k < n; k++) //loop for every k (i.e. phone)
        {
            for (long t = 0; t < outframeNum; t++) //loop for every t
            {
                if (!revert)
                {
                    size_t lastf = 0;
                    for (int f = 0; f < factor; f++)
                    {
                        ...
                        {
                            us(k + n * f, (oututtBeginFrameId + t) * numParallelSequences + uttFrametoChanId) = in1(k, (uttBeginFrameId + t * factor + f) * numParallelSequences + uttFrametoChanId);
                            lastf = f;
                        }
                        else
                ...
                else
                {
                    for (int f = 0; f < factor; f++)
                    {
                        if (t * factor + f < frameNum)
                        {
                            us(k, (uttBeginFrameId + t * factor + f) * numParallelSequences + uttFrametoChanId) = in1(k + n * f, (oututtBeginFrameId + t) * numParallelSequences + uttFrametoChanId);
                        }
                    }
                }
            }
