...
template <typename DType>
inline std::shared_ptr<MKLDNNData<DType>> mkl_get_mem_desc(const std::shared_ptr<MKLMemHolder> data_mem)
{
    std::shared_ptr<PrvMemDescr> prv_descriptor = data_mem->get_prv_descriptor();
    assert(prv_descriptor->get_descr_type() == PrvMemDescr::PRV_DESCR_MKLDNN);
    std::shared_ptr<MKLDNNData<DType>> mem_descr = std::static_pointer_cast<MKLDNNData<DType>>(prv_descriptor);
...
struct attr_t
{
    struct scale_t
    {
        ...
        scale_t(float s, policy_t p = NONE) : scale(s)
        {
            policy = p;
        }
        ...
        if (!pops.is_def())
        {
            ...
            for (int idx = 0; idx < pops.len; ++idx)
            {
                ...
                case attr_t::post_ops_t::RELU:
                    mkldnn_pops.append_eltwise(e.eltwise.scale, mkldnn::algorithm::eltwise_relu, e.eltwise.alpha,
                                               e.eltwise.beta);
                    break;
                default:
    ...
    attr_t(mkldnn::round_mode rm, float s, scale_t::policy_t p = scale_t::policy_t::NONE)
        : rmode(rm), oscale(s, p), mkldnn_attr()
    {
    }

    attr_t() : rmode(mkldnn::round_mode::round_nearest), oscale(1.0), mkldnn_attr() {}
    ...
    bool is_def() const
    {
        return true && rmode == mkldnn::round_mode::round_nearest && oscale.is_def();
    }
};
} // namespace CNTK
} // namespace MSR
