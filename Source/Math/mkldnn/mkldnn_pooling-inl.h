...
public:
    ...
    explicit MKLDNNPoolingOp(ConvolveGeometryPtr geometry, ImageLayoutKind imageLayout, PoolKind kind,
                             bool poolIncludePad)
        : MKLDNNLayer<Dtype>(),
          num_(0),
          channels_(0),
          ...
          pad_l_w_(0),
          pad_r_w_(0),
          poolingFwdInference_pd(NULL),
          poolingFwdTraining_pd(NULL),
          poolingBwd_pd(NULL)
    {
        m_geometry = geometry;
...
private:
    void LayerSetUp(int batchSize)
    {
        ...
        this->pad_r_h_ = (this->height_out_ - 1) * this->stride_h_ - this->pad_l_h_ - this->height_ + this->kernel_h_;
        this->pad_r_w_ = (this->width_out_ - 1) * this->stride_w_ - this->pad_l_w_ - this->width_ + this->kernel_w_;
    }
...
public:
    void InitPoolingFwd(const Mat& in)
    {
        ...
        bool bottom_data_is_prv = (const_cast<Dtype*>(mkl_prv_data<Dtype>(in)) != NULL);
        ...
        std::shared_ptr<mkldnn::memory::desc> init_fwd_bottom_md(new mkldnn::memory::desc({bottom_tz}, mpcsn, cmfmt));
        std::shared_ptr<mkldnn::memory::desc> init_fwd_top_md(new mkldnn::memory::desc({top_tz}, mpcsn, cmfmt));
        std::shared_ptr<MemPD> usr_bottom_data_mpd(new MemPD({{bottom_tz}, mpcsn, mfmt_nchw}, cpu_engine));
        std::shared_ptr<MemPD> usr_top_data_mpd(new MemPD({{top_tz}, mpcsn, mfmt_nchw}, cpu_engine));

        mkldnn::pooling_forward::desc poolingFwdInference_desc(
            mkldnn::prop_kind::forward_scoring, m_pooling_algorithm, *init_fwd_bottom_md, *init_fwd_top_md, {sh, sw},
            {kh, kw}, {pt, pl}, {pb, pr}, mkldnn::padding_kind::zero);
        mkldnn::pooling_forward::desc poolingFwdTraining_desc(mkldnn::prop_kind::forward_training, m_pooling_algorithm,
                                                              *init_fwd_bottom_md, *init_fwd_top_md, {sh, sw}, {kh, kw},
                                                              {pt, pl}, {pb, pr}, mkldnn::padding_kind::zero);
        poolingFwdInference_pd.reset(new mkldnn::pooling_forward::primitive_desc(poolingFwdInference_desc, cpu_engine));
        assert(poolingFwdInference_pd);
        poolingFwdTraining_pd.reset(new mkldnn::pooling_forward::primitive_desc(poolingFwdTraining_desc, cpu_engine));
        ...
        {
            indices_pd.reset(new mkldnn::memory::primitive_desc(poolingFwdTraining_pd->workspace_primitive_desc()));
    ...
    virtual void Forward(const Mat& in, Mat& out, bool inferenceOnly)
    {
        ...
        {
            poolingFwd.reset(
                new mkldnn::pooling_forward(*poolingFwdInference_pd, *fwd_input_primitive, *fwd_output_memory));
        }
        poolingFwd.submit();
    }
    void InitPoolingBwd(const Mat& out)
    {
        ...
        int32_t pr = this->pad_r_w_;
        int32_t pl = this->pad_l_w_;
        void* top_data = const_cast<Dtype*>(mkl_prv_data<Dtype>(out));
        ...
        if (top_is_prv)
        {
            std::shared_ptr<MKLDNNMemoryDescriptor<Dtype>> mem_descr = get_mkldnn_prv_descriptor<Dtype>(out);
        ...
        std::shared_ptr<mkldnn::memory::desc> init_bwd_top_md(new mkldnn::memory::desc({top_tz}, mpcsn, bwd_cmfmt));
        std::shared_ptr<MemPD> usr_bottom_data_mpd(new MemPD({{bottom_tz}, mpcsn, mfmt_nchw}, cpu_engine));
        std::shared_ptr<MemPD> usr_top_data_mpd(new MemPD({{top_tz}, mpcsn, mfmt_nchw}, cpu_engine));
        // ---- Initialize pooling primitive descriptor -------------
        mkldnn::pooling_backward::desc poolingBwd_desc(this->m_pooling_algorithm, *init_bwd_bottom_md, *init_bwd_top_md,
                                                       {sh, sw}, {kh, kw}, {pt, pl}, {pb, pr},
                                                       mkldnn::padding_kind::zero);
        poolingBwd_pd.reset(
            new mkldnn::pooling_backward::primitive_desc(poolingBwd_desc, cpu_engine, *poolingFwdTraining_pd));
    ...
    virtual void Backward(const Mat& out, const Mat& srcGrad, const Mat& in, Mat& grad)
    {
        ...
        {
            poolingBwd.reset(
                new mkldnn::pooling_backward(*poolingBwd_pd, *diff_dst_memory, *indices_memory, *diff_src_memory));
        }
        else
        {
            poolingBwd.reset(new mkldnn::pooling_backward(*poolingBwd_pd, *diff_dst_memory, *diff_src_memory));
        }
        poolingBwd.submit();
    }
...
private:
    ...
    std::shared_ptr<MKLDNNData<Dtype>> fwd_bottom_data, fwd_top_data, bwd_top_diff, bwd_bottom_diff;
...
template <>
int MKLDNNPoolingOp<float>::s_id_gen = 1;
template <>
int MKLDNNPoolingOp<double>::s_id_gen = 1;
} // namespace CNTK
} // namespace MSR
} // namespace Microsoft
#endif
