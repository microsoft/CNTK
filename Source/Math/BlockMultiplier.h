	...
	static void BlockHandler128x4Thread(HandlerArgs<BlockHandlerT> ha)
	{
		...
		int n = ha.n;
		{
			...
			for (int c = 0; c < n; ++c)
			{
				...
				//__m128i firstHorizontal = _mm_hadd_epi32(result1, result1);
...
//#pragma omp parallel for
		for (int currBlock = 0; currBlock < ha.blocks; currBlock += blocksAtOnce)
		{
			//BlockHandlerT::HandleBlock128(currBlock, ha.startRow, ha.k, ha.n, ha.newA, ha.B, ha.transC, 
			//	std::min(ha.blocks - currBlock, blocksAtOnce), resultStorage);
...
public:
	...
	//The resulting code results in matrix multiplies that are  few percent slower than the non-saturating code,
...
//temp reference kernel to get the algo right
	...
	int n, int blockSize, int* kOffset)
{
	...
	//Bump offset for nexe level down
	*kOffset += (numBlocks * blockSize);
	//Return ptr to next value to be written
	return curr;
}

//Version 2: Instead of just reordering B, we can also reorder A, pulling in multiple rows at once.
	...
	for (int startRow = 0; startRow < m; startRow += rowsPerBlock)
	{
		for (int currBlock = 0; currBlock < blocksPerRow; ++currBlock)
		{
			for (int rowOffset = 0; rowOffset < rowsPerBlock; ++rowOffset)
			{
				...
				{
					//*currOut++ = A[RowColToOffset(*pKOffset + startRow + rowOffset, startCol + c, k)];
...
//Reviewers: There is a lot of repetitive code here that could be cleaned up by having the c'tor
//specify a list of block sizes. I'm not sure whether this is worth doing or not.
