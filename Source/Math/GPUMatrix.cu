                                                          ...
                                                          const bool isColWise)
{
    ...
    if (isColWise)
    {
        ...
        size_t numSequences = uttFrameNum.size();
        //int numSequences = uttFrameToChanInd.size();
        // Max number of phones in utterances in this minibatch
        //size_t maxPhoneNum = phoneSeq.GetNumRows();

        ElemType* uttdata = new ElemType[numSequences];
        GPUMatrix<ElemType> uttInfo(prob.GetComputeDeviceId());
        ...
        */
        ...
        size_t* gpuPhoneToChanInd;
        CUDA_CALL(cudaMalloc((void**) &gpuPhoneToChanInd, uttNum * sizeof(size_t)));
        CUDA_CALL(cudaMemcpy(gpuPhoneToChanInd, uttPhoneToChanInd.data(), uttNum * sizeof(size_t), cudaMemcpyHostToDevice));
        ...
        ElemType zerVar = 0.0;
        totalScore.SetColumn(&zerVar, 0);
        _assignRNNTTotalScore<<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, t_stream>>>(beta.Data(), totalScore.Data(), numSequences, uttInfo.Data());
        ...
        {
            _assignRNNTScoreS1<<<block_tail, thread_tail, 0, t_stream>>>(gpuDerivativeValue, alpha.Data(), beta.Data(), uttFrameNum[s], uttPhoneNum[s],
        ...
        //size_t callTimes = 0;
        while (sumYdim < totalUTNum)
        {
            int Ydim = (int) min((float) maxSTU, (float) ((int) totalUTNum - sumYdim + DEFAULT_THREAD_PER_DIM - 1) / DEFAULT_THREAD_PER_DIM * DEFAULT_THREAD_PER_DIM);
            dim3 block_tail2(Ydim / DEFAULT_THREAD_PER_DIM, (totalPhoneNum + DEFAULT_THREAD_PER_DIM - 1) / DEFAULT_THREAD_PER_DIM);
            _assignRNNTScoreS2_speed2<<<block_tail2, thread_tail, 0, t_stream>>>(Data(), gpuDerivativeValue, alpha.Data(), beta.Data(), matrixPhoneSeq.Data(), uttInfo.Data(),
                                                                                 maxPhoneNum, totalPhoneNum, blankTokenId, numSequences, totalUTNum, sumYdim);
            sumYdim += Ydim;
        }
        ...
        CUDA_CALL(cudaFree(gpuPhoneToChanInd));
...
GPUMatrix<ElemType>& GPUMatrix<ElemType>::AssignUserOp1(GPUMatrix<ElemType>& in1, GPUMatrix<ElemType>& in2, GPUMatrix<ElemType>& uttInfo, const size_t totalcol, const size_t numParallelSequences, const size_t numPhoneParallelSequences)
{
    ...
    int maxSTU = MAX_TOTAL_THREAD / BS / DEFAULT_THREAD_PER_DIM * DEFAULT_THREAD_PER_DIM;
...
GPUMatrix<ElemType>& GPUMatrix<ElemType>::AssignUserOp2(GPUMatrix<ElemType>& in1, GPUMatrix<ElemType>& uttInfo, const size_t numParallelSequences, const size_t numPhoneParallelSequences, const size_t maxFrameNum, const size_t maxPhoneNum, const size_t Idx)
{
    ...
    // Ensure that we allocate correct number of blocks for given number of utterances and max number of phones in those utterances
    dim3 block_tail((nRow + DEFAULT_THREAD_PER_DIM - 1) / DEFAULT_THREAD_PER_DIM, (nCol + DEFAULT_THREAD_PER_DIM - 1) / DEFAULT_THREAD_PER_DIM, uttNum);
    in1.PrepareDevice();
    SyncGuard syncGuard;
    SetValue(0.0);
