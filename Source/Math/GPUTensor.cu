...
struct FixedArray
{
    ...
    __device__ __host__ T& back()       { return m_data[N-1]; }
    __device__ __host__ T  back() const { return m_data[N-1]; }
    template <class VEC>
...
struct FixedArray<T, 0>
{
    ...
    __device__ __host__ T& operator[](size_t n); // should never be called
    __device__ __host__ T  operator[](size_t n) const;
    __device__ __host__ T& back();
    __device__ __host__ T  back() const;
    template <class VEC>
...
struct FixedMatrix<T, N, 0>
{
    ...
    __device__ __host__ T& operator()(size_t n, size_t k); // should never be called
...
// -----------------------------------------------------------------------
// helper functions to locate elements
// -----------------------------------------------------------------------

// increment one axis by a number of index steps
// Can be used with the actual index (from unincremented pointers) or with 1 to do one-step increments in a loop.
template<class ElemType, C_size_t NUM_ARGS, C_int RANK, C_size_t NUM_ARGS_TO_BUMP>
static __device__ void IncrementPointersAlongAxis(C_size_t axis, C_size_t indexIncrement,
                                                  FixedArray<ElemType*, NUM_ARGS>& /*in/out*/ pointers,
                                                  const FixedMatrix<C_int, NUM_ARGS, RANK>& strides)
{
#pragma unroll // apply this indexIncrement to the pointers
    for (C_size_t i = 0; i < NUM_ARGS_TO_BUMP; i++)
        pointers[i] += indexIncrement * strides(i, axis);
}
...
static __device__ bool LocateElementFromThreadId(CUDA_LONG id, FixedArray<ElemType*, NUM_ARGS>& /*in/out*/ pointers,
                                                 const FixedMatrix<C_int, NUM_ARGS, RANK>& strides,
...
#pragma unroll
    for (auto axis = (C_size_t)RANK; axis --> 0; )
    {
        ...
        IncrementPointersAlongAxis<ElemType, NUM_ARGS, RANK, /*NUM_ARGS_TO_BUMP=*/NUM_ARGS>(axis, index, pointers, strides);
...
// This requires a nested loop, with nesting depth = REDUCTION_RANK.
// This is implemented by a recursive function template. Since C++ does not allow
// partial specialization, we must detour via a struct template.
// -----------------------------------------------------------------------
...
struct ReduceWithNestedLoops
{
                                                            ...
                                                            const FixedMatrix<C_int, NUM_ARGS, REDUCTION_RANK>& reducingStrides)
    {
        ...
        {
            // bump the pointers (NUM_ARGS-1 because output is not used here)
            IncrementPointersAlongAxis<ElemType, NUM_ARGS, REDUCTION_RANK, /*NUM_ARGS_TO_BUMP=*/NUM_ARGS - 1>((C_size_t)REDUCTION_AXIS, /*indexIncrement=*/1, pointers, reducingStrides);
            // do the next iteration along this axis
...
// We also get here if the operation has no reduction.
// The pointers are pointing to the input element.
template <class ElemType, C_size_t NUM_ARGS, C_int REDUCTION_RANK>
struct ReduceWithNestedLoops<ElemType, NUM_ARGS, REDUCTION_RANK, /*REDUCTION_AXIS=*/-1>
{
...
// Similar to ReduceWithNestedLoops but count the number of elements seen so far and keep track
                                            ...
                                            const FixedArray<fast_divmod, REGULAR_RANK>& regularOpStrideDivmod, const FixedArray<fast_divmod, REDUCTION_RANK>& reducingOpDimDivmod)
{
    ...
    //  - the output pointers point to the right element (input pointers may still iterate for reduction)
    if (!PARALLEL_REDUCE)
    {
    ...
    else // reduction using parallel threads. We will only get here if REDUCTION_RANK > 0.
    {
#undef ALLOW_ATOMIC_REDUCTION // define to enable use of atomicAdd() below, for comparing it (note: this may no longer compile or work)
        ...
        // aggregate all elements within this CUDA thread (and below we will aggregate across the CUDA threads)
...
// -----------------------------------------------------------------------
// perform loop over regular index REGULAR_AXIS for NUM_ARGS-nary operations (NUM_ARGS counting the output)
// keep track of the indices.
// -----------------------------------------------------------------------

// The 'pointers' only refer to a single element, so we will bump them in-place to perform indexing.
template <class ElemType, C_size_t NUM_ARGS, C_int REDUCTION_RANK, C_int REGULAR_RANK, C_int REGULAR_AXIS>
struct TensorArgOpElement
{
    ...
    static __device__ void Compute(CUDA_LONG id, FixedArray<ElemType*, NUM_ARGS>& pointers,
                                   ...
                                   const FixedArray<fast_divmod, REGULAR_RANK>& regularOpStrideDivmod, const FixedArray<fast_divmod, REDUCTION_RANK>& reducingOpDimDivmod)
    {
        // map id (location on grid) to index[REGULAR_AXIS]
#ifndef USE_FAST_DIVMOD
...
#pragma unroll
        ...
        TensorArgOpElement<ElemType, NUM_ARGS, REDUCTION_RANK, REGULAR_RANK, REGULAR_AXIS - 1>::Compute(
            id, pointers, reductionOp, regularOpStrides, regularStrides, reducingOpDims, reducingStrides, reductionBegin, reductionChunkSize,
            regularOpStrideDivmod, reducingOpDimDivmod);
    }
};

// specialization for REGULAR_AXIS=-1 terminates the template recursion, and computes reductions in a for loop
template <class ElemType, C_size_t NUM_ARGS, C_int REDUCTION_RANK, C_int REGULAR_RANK>
struct TensorArgOpElement<ElemType, NUM_ARGS, REDUCTION_RANK, REGULAR_RANK, /*REGULAR_AXIS=*/-1>
{
                                ...
                                FixedArray<fast_divmod, REGULAR_RANK> regularOpStrideDivmod, FixedArray<fast_divmod, REDUCTION_RANK> reducingOpDimDivmod)
{
    ...
    if (id < numElements) // note: there are no __syncthread() calls inside
        ComputeOutputElement<ElemType, NUM_ARGS, REDUCTION_RANK, REGULAR_RANK, /*PARALLEL_REDUCE=*/false>(
            id, beta, pointers,
            alpha, op, reductionOp, regularOpStrides, regularStrides, reducingOpDims, reducingStrides,
            /*reductionBegin=*/ 0, /*reductionChunkSize=*/ 0,
            regularOpStrideDivmod, reducingOpDimDivmod);
}
                                   ...
                                   FixedArray<fast_divmod, REGULAR_RANK> regularOpStrideDivmod, FixedArray<fast_divmod, REDUCTION_RANK> reducingOpDimDivmod)
{
    CUDA_LONG id = GridDim::GetLinearThreadId();
    if (id < numElements) // note: there are no __syncthread() calls inside
        TensorArgOpElement<ElemType, NUM_ARGS, REDUCTION_RANK, REGULAR_RANK, /*REGULAR_AXIS=*/REGULAR_RANK - 1>::Compute(
            id, pointers,
                           ...
                           const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, NUM_ARGS>& regularStrideVectors)
{
    ...
    for (C_size_t regularAxis = 0; regularAxis < regularOpDims.size(); regularAxis++)
    {
        ...
        numElements *= (C_size_t) regularOpDims[regularAxis];
    }
                                             ...
                                             FixedArray<fast_divmod, REGULAR_RANK> regularOpStrideDivmod, FixedArray<fast_divmod, REDUCTION_RANK> reducingOpDimDivmod)
{
    ...
    CUDA_LONG reductionBlock = blockIdx.z;                         // reduction-block index  --larger reductions are split into blocks
    pointers.back() += numElements * reductionBlock; // the output tensor is dense (no gaps); and there is one copy for each reduction block (those get further reduced into one later)
#endif
    if (id < numElements)                               // note: we have __syncthread() calls but only entire blocks in sync, so this is OK
        ComputeOutputElement<ElemType, NUM_ARGS, REDUCTION_RANK, REGULAR_RANK, /*PARALLEL_REDUCE=*/true>(
            id, beta, pointers,
                                        ...
                                        const SmallVector<size_t>& reducingOpDimVector, const array<SmallVector<ptrdiff_t>, NUM_ARGS>& reducingStrideVectors)
{
    ...
    SmallVector<fast_divmod> reducingOpDimDivmodVector;
    C_size_t stride = 1;
    for (C_size_t regularAxis = 0; regularAxis < reducingOpDimVector.size(); ++regularAxis)
    {
        reducingOpDimDivmodVector.push_back(fast_divmod(stride));
        stride *= (C_size_t)reducingOpDimVector[regularAxis];
    }
    ...
    for (C_size_t regularAxis = 0; regularAxis < reducingOpDimVector.size(); regularAxis++)
        reductionDim *= (C_size_t) reducingOpDimVector[regularAxis];
    GridDim grid(NN);
    ...
    else
    {
        ...
        else
        {
            ...
            for (size_t regularAxis = 0; regularAxis < regularOpStrides.size(); regularAxis++)
                regularStrideVectors1[NUM_ARGS - 1][regularAxis] = (ptrdiff_t)regularOpStrideVector[regularAxis];
...
// tensor operation with REGULAR_RANK regular dimensions (0 means scalar)
template <class ElemType, C_size_t NUM_ARGS, C_int REGULAR_RANK>
...
// This function expands into different REGULAR_RANK template parameters. It also eliminates the offsets by adding them to the pointers.
