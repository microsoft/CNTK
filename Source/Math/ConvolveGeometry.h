...
class ConvolveGeometry final {
 public:
  ...
  ConvolveGeometry(const TensorShape& inputShape, const TensorShape& kernelShape, const TensorShape& mapCount, const TensorShape& stride,
      ...
      : m_inputShape(inputShape), m_kernelShape(kernelShape), m_mapCount(mapCount), m_stride(stride), m_sharing(sharing), m_autoPad(autoPad), m_lowerPad(lowerPad), m_upperPad(upperPad), m_dilation(dilation), m_groups(groups) {
  ...
  int GetUpperPad(size_t dim) const {
    ...
    int extra = inpSize - cells;
    int center = extra / 2;
    return (kernSize - 1) - (kernSize - 1) / 2 - center;
  }
