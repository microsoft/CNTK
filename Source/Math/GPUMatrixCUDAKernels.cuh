    ...
    ElemType* out)             // D x T, left filter gradient
{
    ...
    if (steps > 1)
    {
        if (col >= 0 && abs(flags[threadIdx.x] - flags[col])<1e-2)
        {
        ...
        for (int i = 1; i<steps; ++i)
        {
            ...
            if (index < cols && abs(flags[index] - flags[index - shift])<1e-2)
    ...
    {
        if (col >= 0 && threadIdx.x<cols && abs(flags[threadIdx.x] - flags[col])<1e-2)
        {
            aux[threadIdx.x] = in[row + col*rows] * diff[row + threadIdx.x*rows];
        }
        else
    ...
    ElemType* out)             // D x T, right filter gradient
{
    ...
    if (steps > 1)
    {
        if (col < cols && abs(flags[threadIdx.x] - flags[col])<1e-2)
        {
            aux[threadIdx.x] = in[row + col*rows] * diff[row + threadIdx.x*rows];
        }
        else
        ...
        for (int i = 1; i<steps; ++i)
        {
            ...
            if (index + shift < cols && abs(flags[index] - flags[index + shift])<1e-2)
                aux[threadIdx.x] += in[(index + shift)*rows + row] * diff[index*rows + row];
        }
        __syncthreads();
    }
    ...
    {
        if (col < cols && threadIdx.x<cols && abs(flags[threadIdx.x] - flags[col])<1e-2)
        {
            aux[threadIdx.x] = in[row + col*rows] * diff[row + threadIdx.x*rows];
        }
        else
