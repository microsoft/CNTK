...
template <class ElemType>
void TensorView<ElemType>::SparseAssignCopyOf(const TensorView& a, size_t* NzOffset, const size_t RowOffset)
{
    bool transA = false;
    bool transB = false;
...
template <class ElemType>
/*static*/ void TensorView<ElemType>::AddSparseNumOfNZs(const TensorView& a, size_t* numNZs)
{
    bool transA = false;
    // determine integration dimension offset
    auto shapeA = a.m_shape;
...
template <class ElemType>
void TensorView<ElemType>::AddSparseColumnIndex(const TensorView& a)
{
    bool transA = false;
    bool transB = false;
    ...
    let  A = a.Reshaped(shapeA).AsMatrix();
    auto B = Reshaped(shapeB).AsMatrix();
    Matrix<ElemType>::AddSparseColumnIndex(*B, *A);
}

template <class ElemType>
void TensorView<ElemType>::ResizeAsSparseMatrix(const size_t numRows, const size_t numCols, const size_t numNZs)
{
    bool transA = false;
    // determine integration dimension offset
    auto shapeA = m_shape;
