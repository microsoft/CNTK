...
/*virtual*/ void WhereNode<ElemType>::ForwardPropNonLooping() /*override*/
{
    ...
    let& inMBLayout = InputPtr(0)->GetMBLayout();
    let& input = InputPtr(0)->Value();
...
{
    let& sourceMBLayout = InputPtr(SOURCEDATA)->GetMBLayout(); // only used for index conversion
    let& indexMBLayout = InputPtr(INDEXDATA)->GetMBLayout();
    let&  index = InputPtr(INDEXDATA)->Value(); // per-seq index values that are to be mapped
...
{
    InputPtr(INDEXDATA)->MaskMissingValueColumnsTo(FrameRange(InputPtr(INDEXDATA)->GetMBLayout()), -1); // indicates an invalid column to Gather/Scatter
    let&  index  = InputPtr(INDEXDATA)->Value();  // column indices to copy from
    let&  source = InputPtr(SOURCEDATA)->Value(); // source data to copy
...
/*virtual*/ void GatherPackedNode<ElemType>::BackpropToNonLooping(size_t inputIndex) /*override*/
{
    ...
    {
        let&  index          = InputPtr(INDEXDATA)->Value();     // column indices to copy from
        auto& sourceGradient = InputPtr(SOURCEDATA)->Gradient(); // source to propagate the gradient intpu
...
{
    if (*InputPtr(INDEXDATA)->GetMBLayout() != *InputPtr(SOURCEDATA)->GetMBLayout())
...
/*virtual*/ void ScatterPackedNode<ElemType>::BackpropToNonLooping(size_t inputIndex) /*override*/
{
    ...
    {
        let&  index          = InputPtr(INDEXDATA)->Value();     // column indices to copy from
        auto& sourceGradient = InputPtr(SOURCEDATA)->Gradient(); // source to propagate the gradient input
        auto& outputGradient =                    Gradient(); // output gradient to propagate
