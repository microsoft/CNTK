...
void ComputationNode<ElemType>::LazyZeroGradient(const ComputationNodeBase* gradientInitializedBy)
{
    ...
    if (gradientInitializedBy == nullptr)
        LogicError("%ls %ls operation: LazyZeroGradient() called without gradientInitializedBy.", NodeName().c_str(),
                   OperationName().c_str());

    if (m_gradientInitializedBy != nullptr)
        return;

    // gradient optimization to allow parent to overwrite/be reused by non-looping child's gradient instead of
    // accumulating We cannot enable the gradient overwrite/reuse optimization if this node's parent has this same node
    // as multiple of its inputs since, in that case the gradients will flow back from multiple paths of the same parent
    // into the input nor can we apply gradient optimization for nodes in loop as the gradient needs to be accumulated
    // through time steps

    const auto& inputs = gradientInitializedBy->GetInputs();

    if (Globals::ShouldOptimizeGradientAccumulation() && !IsPartOfLoop() &&
...
template <class ElemType>
void ComputationNode<ElemType>::Backprop(const FrameRange& fr, bool childrenInThisLoop,
                                         bool childrenInOuterLoop) /*override*/
{
    ...
    if (fr.IsAllFrames() && IsPartOfLoop() && childrenInThisLoop)
        LogicError("%ls %ls operation: Backprop called with whole-batch FrameRange on node that participates in a loop",
                   NodeName().c_str(), OperationName().c_str());

    for (size_t i = 0; i < m_inputs.size(); i++)
    {
        ComputationNodeBasePtr child = m_inputs[i];
        if (child->NeedsGradient() && ((childrenInThisLoop && child->IsPartOfLoop() == IsPartOfLoop()) ||
                                       (childrenInOuterLoop && child->IsPartOfLoop() != IsPartOfLoop())))
        {
            // fprintf(stderr, "Backprop: %ls %ls operation -> child %d %ls %ls\n", NodeName().c_str(),
            // OperationName().c_str(), (int)i, child->NodeName().c_str(), child->OperationName().c_str());
            if (!NeedsGradient())
...
#endif
            SMART_NODE_INVOKE(ComputationNode, child, LazyZeroGradient,
                              this); // set gradient to 0 if this is the first time
            ...
            {
                LogicError(
                    "Backprop: Inefficiency: %ls %ls operation in loop propagates gradient to non-loop %ls %ls\n",
                    NodeName().c_str(), OperationName().c_str(), child->NodeName().c_str(),
                    child->OperationName().c_str());
            }

            // before backprop, verify gradient optimization info
            SMART_NODE_INVOKE(ComputationNode, child, VerifyGradientOptimization, this);

            // fprintf(stderr, "BackpropTo %d %d %ls %ls\n", (int)fr.timeIdxInSeq, (int)i, NodeName().c_str(),
            // OperationName().c_str());
            ...
            fprintf(stderr, "    [%lu]: %s(%s) (no gradient needed so don't compute for)\n", i,
                    child->OperationName().c_str(), child->NodeName().c_str());
#endif
    }
}

template <class ElemType>
/*static*/ TensorView<ElemType> ComputationNode<ElemType>::Unpack(
    const TensorShape& sampleShape, const Matrix<ElemType>& packedData, const MBLayoutPtr& layout,
    const std::shared_ptr<Matrix<ElemType>>& unpackedDataStorage,
    const std::shared_ptr<Matrix<ElemType>>& tempIndicesStorage, const std::shared_ptr<Matrix<char>>& tempMaskStorage,
    bool batchMajor, const ElemType* gapPadValue)
{
    ...
    if ((maxNumTimeSteps == 1) || (numSequences == 1) ||
        (batchMajor && (layout->GetNumParallelSequences() == layout->GetNumSequences())))
    {
    ...
    else
    {
        ...
        if (!unpackedData)
            unpackedData = std::make_shared<Matrix<ElemType>>(packedData.GetNumRows(), maxNumTimeSteps * numSequences,
                                                              packedData.GetDeviceId(), packedData.GetMatrixType(),
                                                              packedData.GetFormat());
        else
        {
            ...
            if (sequenceInfo.seqId != GAP_SEQUENCE_ID)
            {
                ...
                for (size_t j = 0; j < maxNumTimeSteps; ++j)
                {
                    ...
                    if (j < currentSequenceLength)
                        scatterIndicesVector[((currentSequenceBeginIdx + j) * layout->GetNumParallelSequences()) +
                                             targetParallelStreamIdx] = (ElemType) targetIdx;
                    else
                    {
                        if (gapPadValue)
        ...
        if (!scatterIdxMatrix)
            scatterIdxMatrix = std::make_shared<Matrix<ElemType>>(1, layout->GetNumCols(), scatterIndicesVector.data(),
                                                                  packedData.GetDeviceId());
        else
        ...
        if (gapPadValue && (*gapPadValue != 0))
        {
            ...
            if (!columnsValidityMaskMatrix)
                columnsValidityMaskMatrix = std::make_shared<Matrix<char>>(
                    1, columnsValidityMask.size(), columnsValidityMask.data(), packedData.GetDeviceId());
            else
                columnsValidityMaskMatrix->SetValue(1, columnsValidityMask.size(), packedData.GetDeviceId(),
                                                    columnsValidityMask.data());

            unpackedData->MaskColumnsValue(*columnsValidityMaskMatrix, *gapPadValue,
                                           unpackedData->GetNumCols() / columnsValidityMaskMatrix->GetNumCols());
        }
    }
...
template <class ElemType>
/*static*/ void
ComputationNode<ElemType>::BroadcastToPacked(const Matrix<ElemType>& dataToBroadcast, const MBLayoutPtr& inputLayout,
                                             ElemType beta, Matrix<ElemType>& broadcastTo,
...
#pragma omp parallel for num_threads(NUM_THREADS)
    for (int layoutSequenceIdx = 0; layoutSequenceIdx < numLayoutSequences; ++layoutSequenceIdx)
    {
        ...
        if ((sequenceInfo.seqId != GAP_SEQUENCE_ID) &&
            (targetFrameRange.IsAllFrames() ||
             ((sequenceInfo.tBegin <= (ptrdiff_t)(targetFrameRange.timeIdxInSeq + targetFrameRange.m_timeOffset)) &&
              (sequenceInfo.tEnd > (targetFrameRange.timeIdxInSeq + targetFrameRange.m_timeOffset)))))
        {
    ...
    if (!gatherIdxMatrix)
        gatherIdxMatrix = std::make_shared<Matrix<ElemType>>(1, broadcastTo.GetNumCols(), gatherIndicesVector.data(),
                                                             broadcastTo.GetDeviceId());
    else
...
void ComputationNodeBase::ValidateMBLayout(const ComputationNodeBasePtr which,
                                           const ComputationNodeBasePtr vsWhich) const
{
    ...
    // By allowing this, users will not need ReconcileDynamicAxis() for reductions over a sequence like
    // BS.Sequences.Last().
    if (GetEnvironmentPtr() && (Environment().traceLevel > 0))
    {
        fprintf(stderr,
                "WARNING: %ls: Dynamic axes mismatch between %ls and %ls. If they are incompatible, this will fail "
                "later.\n",
...
void ComputationNodeBase::InferMBLayoutFromInputsForStandardCase(bool isFinalValidationPass)
{
    ...
    {
        if (!input) // node not set yet (DelayedValueNodeBase seems to allow this)--BUGBUG: Then this function won't
                    // operate correctly.
            ;
...
// If allowBroadcast then one can be a sub-dimension of the other (if layout then only for rows, otherwise for cols,
// too). This also helpfully resizes the children if not yet sized.
void ComputationNodeBase::ValidateBinaryZip(bool isFinalValidationPass, bool allowBroadcast)
{
    ...
    for (size_t k = 0; k < shape1.GetRank(); k++)
    {
        ...
        if (dims[k] <= 1 && dim1 != 0)                     // is [0] broadcasting (1) or unspecified (0)?
            ...
            InvalidArgument("%ls: Input dimensions [%s] and [%s] are not compatible.", NodeDescription().c_str(),
                            string(shape0).c_str(), string(shape1).c_str());
    }
...
void ComputationNodeBase::ValidateNaryZip(bool isFinalValidationPass, bool allowBroadcast, size_t numInputs)
{
    ...
    for (size_t k = 0; k < maxRank; k++)
    {
        ...
        for (size_t i = 0; i < numInputs; i++)
        {
            ...
            // make sure that the rank of this input is bigger than the current index (otherwise, these are implied
            // singleton dimensions that do not need to be checked)
            if (currentRank > k)
            {
                size_t currentDim = currentShape[k];
                if (currentDim > 1 && maxDim != currentDim &&
                    maxDim > 1) // 1=broadcasting, 0=not known yet, meant to be inferred
                {
...
void ComputationNodeBase::ValidateUnaryReduce(bool isFinalValidationPass, bool keepDimensions)
{
    ...
    SetDims(keepDimensions ? m_inputs[0]->GetSampleLayout() : (TensorShape::Scalar(Environment().IsV2Library())),
            false);
}

// binary reduce-to-(1,1) operation, e.g. CrossEntropyWithSoftmaxNode
// Currently only called by criterion nodes.
// This function also infers child LearnableParameters. In case you wonder why this is needed for criterion nodes, there
// are edge cases, e.g. a learnable parameter being regularized by a criterion node, where the learnable parameter is
// fed both into that criterion node and other places.
void ComputationNodeBase::ValidateBinaryReduce(bool isFinalValidationPass)
{
    ...
    if (isFinalValidationPass)
    {
        if (!(Input(0)->GetSampleLayout().IsElementwiseCompatibleWith(Input(1)->GetSampleLayout())))
        {
            ...
            LogicError("%ls: The tensor dimensions in the inputs do not match. %s != %s", NodeDescription().c_str(),
                       s1.c_str(), s2.c_str());
        }
...
void ComputationNodeBase::ValidateInferNaryInputDims(size_t numInputs)
{
    // limited inference of children dimensions
    // if dimension not specified we assume two operands' dimensions should be the same
    // NOTE: The assert is set to check if >= numInputs since this is called from nodes which have more than 'nInputs'
    // children.
    //      The number of children is formally verified elsewhere, so this will not break consistency.
    assert(m_inputs.size() >= numInputs);
...
// 'Sample tensor' means we only consider single samples. If we have an MBLayout, that is the sample layout of a single
// matrix column.
// TODO: Turn rank into a member variable, and call this method once in validation (currently called for every single
// ForwardProp/BackpropTo()).
...
{
    // If we have an MB layout then add the necessary sequence and time axes. If we have none, then absorb the column
    // dimension.
    TensorShape tensorShape = GetSampleLayout(); // TODO: Do we need to expect this tensor to have arbitrary strides? In
                                                 // case it came out of a Slice, Reshape, or Transpose op in-place?
    if (HasMBLayout())
    {
...
// To enable its first argument to have an MBLayout, it needs to un-pad if we have an MBLayout but only refer to a
// single sequence and time step.
TensorShape ComputationNodeBase::GetOneSampleTensorSliceFor(size_t rank, const FrameRange& fr) const
{
    ...
    if (HasMBLayout())
        result.TrimRankInPlace(
            rank); // Note: This function will verify once again that the extra dimensions have been reduced to [1 x 1]
    return result;
}
...
{
    return msra::strfun::strprintf("[%s%s%ls]", string(m_sampleLayout).c_str(), HasMBLayout() ? " x " : "",
...
/*virtual*/ void ComputationNode<ElemType>::BeginForwardProp()
{
    ...
    if ((!IsLeaf() || Is<RandomDistributionNode<ElemType>>()) &&
        !RequiresPreCompute()) // TODO: guard this through overrides instead
...
/*virtual*/ void ComputationNode<ElemType>::EndForwardProp()
{
    ...
    {
        MaskMissingValueColumnsToZero(
            FrameRange(m_pMBLayout)); // HasNaN() operates on a whole matrix, so first flatten all gaps to 0
        if (Value().HasNan("EndForwardProp"))
            LogicError("%ls %ls operation unexpectedly produced NaN values.", NodeName().c_str(),
                       OperationName().c_str());

        InvalidateMissingValueColumns(
            FrameRange(m_pMBLayout)); // blast NaNs into columns that are gaps in a packed layout
    }
...
/*virtual*/ void ComputationNode<ElemType>::BeginBackprop()
{
    ...
    if (NeedsGradient())
    {
        ...
        auto VerifyValueShape = [](const ComputationNode<ElemType>& node) {
            ...
            {
                LogicError("%ls %ls operation found to have incorrect Value() matrix shape %lu x %lu during backprop; "
                           "expected shape is %lu x %lu. "
                           "This may be due to incorrect memory sharing.",
                           node.NodeName().c_str(), node.OperationName().c_str(), valueMatrix.GetNumRows(),
                           valueMatrix.GetNumCols(), rows, cols);
            }
        };
...
/*virtual*/ void ComputationNode<ElemType>::EndBackprop()
{
    ...
    if (HasEnvironmentPtr() && Environment().trackGapNans)
    {
        for (size_t i = 0; i < m_inputs.size(); i++)
        {
            ...
            {
                SMART_NODE_INVOKE(
                    ComputationNode, child, MaskMissingGradientColumnsToZero,
                    FrameRange(
                        child->GetMBLayout())); // HasNaN() operates on a whole matrix, so first flatten all gaps to 0

                bool hasNan = false;
                SMART_NODE_INVOKE_WITH_RET(ComputationNode, child, Gradient().HasNan, hasNan, "EndBackprop");
                if (hasNan)
                {
                    LogicError("%ls %ls operation unexpectedly produced NaN gradients on its input %ls.",
                               NodeName().c_str(), OperationName().c_str(), child->NodeName().c_str());
                }
            }
        }
...
void ComputationNode<ElemType>::PrintForwardBackwardTime()
{
    ...
    fprintf(stderr, "%-30S forward avg %07fs, backward avg %07fs (fwd# %d|bwd# %d)\n", m_nodeName.c_str(),
            ...
            backwardCount == 0 ? 0 : backwardDuration / backwardCount, forwardCount, backwardCount);
...
template <class ElemType>
/*virtual*/ void ComputationNode<ElemType>::DumpNodeInfo(const bool /*printValues*/, const bool printMetadata,
                                                         File& fstream) const
{
    if (printMetadata)
    {
...
// 'isSparse' will print all non-zero values as one row (non-transposed, which makes sense for one-hot) or column
// (transposed).
template <class ElemType>
void ComputationNode<ElemType>::WriteMinibatchWithFormatting(
    FILE* f, const FrameRange& fr, size_t onlyUpToRow, size_t onlyUpToT, bool transpose, bool isCategoryLabel,
    bool isSparse, const vector<string>& labelMapping, const string& sequenceSeparator, const string& sequencePrologue,
    const string& sequenceEpilogue, const string& elementSeparator, const string& sampleSeparator,
    string valueFormatString, bool outputGradient, bool onlyShowAbsSumForDense,
    std::function<std::string(size_t)> getKeyById) const
{
    ...
    if (!pMBLayout) // no MBLayout: We are printing aggregates (or LearnableParameters?)
    {
        ...
        pMBLayout->Init(
            1, outputValues.GetNumCols()); // treat this as if we have one single sequence consisting of the columns
    ...
    for (size_t s = 0; s < sequences.size(); s++)
    {
        ...
        {
            auto sh =
                msra::strfun::_strprintf<char>("%s%ld", shape.c_str(), (unsigned long long) seqInfo.GetNumTimeSteps());
        ...
        if (isCategoryLabel) // if is category then find the max value and output its index (possibly mapped to a
                             // string)
        {
            if (formatChar == 's') // verify label dimension
            {
                    ...
                    sampleLayout[0] != labelMapping.size()) // if we match the first dim then use that
                {
                    ...
                    if (warnings++ < 5)
                        fprintf(stderr,
                                "write: Row dimension %d does not match number of entries %d in labelMappingFile, not "
                                "using mapping\n",
        ...
        auto print = [&](double dval) {
            if (formatChar == 'f') // print as real number
            {
                if (dval == 0)
                    dval = fabs(
                        dval); // clear the sign of a negative 0, which are produced inconsistently between CPU and GPU
        ...
        let istop = transpose ? onlyUpToRow : onlyUpToT; // we stop at these dimensions (for debugging, one often needs
                                                         // only the first few values of those huge matrices)
        ...
        if (isSparse)
        {
            ...
            for (size_t i = 0; i < iend; i++) // loop over elements --we just flatten them all out
            {
                for (size_t j = 0; j < jend; j++) // loop over rows
                {
                    ...
                    if (dval != 1.0 ||
                        formatChar != 'f') // hack: we assume that we are either one-hot or never precisely hitting 1.0
                        print(dval);
                    ...
                    for (size_t k = 0; k < sampleLayout.size(); k++)
                    {
                        ...
                        if (sampleLayout[k] == labelMapping.size()) // annotate index with label if dimensions match
                                                                    // (which may misfire once in a while)
...
#pragma omp parallel for reduction(+ : absSum)
            ...
            {
                for (size_t j = 0; j < jend;
                     j++) // loop over output rows     --BUGBUG: row index is 'i'!! Rename these!!
                {
                    if (j > 0)
                        fprintfOrDie(f, "%s", sampleSep.c_str());
                    if (j == jstop &&
                        jstop < jend - 1) // if jstop == jend-1 we may as well just print the value instead of '...'
                    {
                        fprintfOrDie(f, "...+%d", (int) (jend - jstop)); // 'nuff said
                        break;
                    }
                    // inject sample tensor index if we are printing row-wise and it's a tensor
                    if (!transpose && sampleLayout.size() > 1 &&
                        !isCategoryLabel) // each row is a different sample dimension
                    {
                        for (size_t k = 0; k < sampleLayout.size(); k++)
                            fprintfOrDie(f, "%c%d", k == 0 ? '[' : ',',
                                         (int) ((j / sampleLayout.GetStrides()[k])) % sampleLayout[k]);
                        fprintfOrDie(f, "]\t");
                    }
...
/*static*/ string WriteFormattingOptions::Processed(const wstring& nodeName, string fragment, size_t minibatchId)
{
    ...
    if (fragment.find("%n") != fragment.npos)
        fragment = msra::strfun::ReplaceAll<string>(fragment, "%n",
                                                    msra::strfun::_strprintf<char>("%ld", minibatchId).c_str());
    // %d: sequenceId
    return fragment;
}
...
WriteFormattingOptions::WriteFormattingOptions(const ConfigRecordType& config) : WriteFormattingOptions()
{
    ...
    if (config.Exists(L"format"))
    {
        ...
        if (formatConfig.ExistsCurrent(L"type")) // do not inherit 'type' from outer block
        {
            ...
            else if (type == L"sparse")
                isSparse = true;
            else
                InvalidArgument("write: type must be 'real', 'category', or 'sparse'");
        ...
        sequenceSeparator =
            msra::strfun::utf8(formatConfig(L"sequenceSeparator", (wstring) msra::strfun::utf16(sequenceSeparator)));
        sequencePrologue =
            msra::strfun::utf8(formatConfig(L"sequencePrologue", (wstring) msra::strfun::utf16(sequencePrologue)));
        sequenceEpilogue =
            msra::strfun::utf8(formatConfig(L"sequenceEpilogue", (wstring) msra::strfun::utf16(sequenceEpilogue)));
        elementSeparator =
            msra::strfun::utf8(formatConfig(L"elementSeparator", (wstring) msra::strfun::utf16(elementSeparator)));
        sampleSeparator =
            msra::strfun::utf8(formatConfig(L"sampleSeparator", (wstring) msra::strfun::utf16(sampleSeparator)));
        precisionFormat =
            msra::strfun::utf8(formatConfig(L"precisionFormat", (wstring) msra::strfun::utf16(precisionFormat)));
...
template <>
map<size_t, map<size_t, shared_ptr<SingleMatrix>>> ComputationNode<float>::s_constOnes{};
template <>
map<size_t, map<size_t, shared_ptr<DoubleMatrix>>> ComputationNode<double>::s_constOnes{};
template <>
map<size_t, map<size_t, shared_ptr<HalfMatrix>>> ComputationNode<half>::s_constOnes{};
...
shared_ptr<Object> MakeRuntimeObject<ComputationNodeBase>(const IConfigRecordPtr configp)
{
    ...
    //    node->Validate(/*isFinalValidationPass*/false); // do an initial validation, so that we have access to
    //    dimensions
    return node;
}

ScriptableObjects::ConfigurableRuntimeTypeRegister::Add<ComputationNodeBase>
    registerComputationNode(L"ComputationNode");
