...
public:
    ...
    {
        ValueFor(fr).AssignValuesOf(InputPtr(0)->ValueFor(fr));
    }

    virtual void /*ComputationNode::*/ BackpropTo(const size_t inputIndex, const FrameRange& fr) override
    {
        InputPtr(inputIndex)->GradientFor(fr).AssignValuesOf(GradientFor(fr));
    }
...
public:
    ...
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        ...
        if (*m_pMBLayout != *InputPtr(0)->GetMBLayout()) // this does a deep value-level comparison
            InvalidArgument("%ls %ls operation discovered that %ls %ls operation produced an MB layout that is incompatible with that of %ls %ls.",
                            NodeName().c_str(), OperationName().c_str(),
                            InputPtr(0)->NodeName().c_str(), InputPtr(0)->OperationName().c_str(),
                            InputPtr(1)->NodeName().c_str(), InputPtr(1)->OperationName().c_str());

        // copy the data from 'dataInput'
        ValueFor(fr).AssignValuesOf(InputPtr(0)->ValueFor(fr.WithLayout(InputPtr(0)->GetMBLayout()))); // just propagate through
    ...
    {
        if (inputIndex == 0)
            InputPtr(0)->GradientFor(fr.WithLayout(InputPtr(0)->GetMBLayout())) += GradientFor(fr);
...
private:
    ...
    size_t BeginIndex() const { return m_beginIndex >= 0 ? (size_t)m_beginIndex : (size_t)(m_beginIndex + InputPtr(0)->GetSampleLayout()[m_axis - 1]); }
    size_t EndIndex()   const { return m_endIndex   >  0 ? (size_t)m_endIndex   : (size_t)(m_endIndex   + InputPtr(0)->GetSampleLayout()[m_axis - 1]); }
    ...
    {
        auto inputSlice = InputPtr(0)->GetTensorSliceFor(rank, fr);    // input must be narrowed down
...
public:
    ...
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        ...
        let   input = TensorView<ElemType>(InputPtr(0)->ValuePtr(), GetInputSlice(rank, fr.AllowBroadcast()));
    ...
    virtual void /*ComputationNode::*/ BackpropTo(const size_t /*inputIndex*/, const FrameRange& fr) override
    {
        ...
        auto inputGrad = TensorView<ElemType>(InputPtr(0)->GradientPtr(), GetInputSlice(rank, fr.AllowBroadcast()));
...
public:
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        ...
        {
            let input = InputPtr(inputIndex)->ValueTensorFor(rank, fr.AllowBroadcast());
    ...
    virtual void /*ComputationNode::*/ BackpropTo(const size_t inputIndex, const FrameRange& fr) override
    {
        ...
        auto inputGrad = InputPtr(inputIndex)->GradientTensorFor(rank, fr.AllowBroadcast());
        let outputSubSlice = NarrowToStripe(outputSlice, inputIndex);
...
public:
    ...
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        ValueFor(fr).AssignRepeatOf(InputPtr(0)->ValueFor(fr), m_numRepeat, 1);
    }

    virtual void /*ComputationNode::*/ BackpropTo(const size_t /*inputIndex*/, const FrameRange& fr) override
    {
        InputPtr(0)->GradientFor(fr).AddToRowRepeatValuesOf(GradientFor(fr), m_numRepeat);
    }
...
public:
    ...
    virtual void /*ComputationNodeNonLooping::*/ ForwardPropNonLooping() override
    {
        InputPtr(0)->ValueAsMatrix().AssignDiagonalValuesTo(ValueAsMatrix()); // TODO: use tensor lib; this is a stride operation
#if NANCHECK
...
public:
    ...
    virtual void /*IComputationNode::*/ BeginForwardProp() override
    {
        ...
        if (m_pMBLayout && factor() != 1)
        {
            ...
            if (weStack())
            {
                ...
                if (InputPtr(0)->GetMBLayout()->GetNumTimeSteps() * InputPtr(0)->GetSampleMatrixNumRows() / m_numTargetRows != 1)
            ...
            else
            {
                ...
                m_pMBLayout->Init(InputPtr(0)->GetNumParallelSequences(), InputPtr(0)->GetMBLayout()->GetNumTimeSteps() * InputPtr(0)->GetSampleMatrixNumRows() / m_numTargetRows);
    ...
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        size_t rows = InputPtr(0)->Value().GetNumRows(), cols = InputPtr(0)->Value().GetNumCols();
        ...
        if (!m_pMBLayout || factor() == 1)
        {
            Value().Reshaped(newCols * m_numTargetRows, 1).AssignValuesOf(InputPtr(0)->Value().Reshaped(cols * rows, 1)); // copy the values as one long vector
        }
        ...
        else
        {
            ...
            if (weStack())
                Base::Stack(fr, m_pMBLayout, InputPtr(0)->Value(), Value(), factor(), false /*addTo*/);
            else
                Base::Unstack(fr.WithLayout(InputPtr(0)->GetMBLayout()), InputPtr(0)->GetMBLayout(), InputPtr(0)->Value(), Value(), factor(), false /*addTo*/);
        }
    }
    ...
    virtual void /*ComputationNode::*/ BackpropTo(const size_t /*inputIndex*/, const FrameRange& fr) override
    {
        ...
        if (!m_pMBLayout || factor() == 1)
        {
            InputPtr(0)->Gradient().Reshaped(cols * rows, 1) += Gradient().Reshaped(newCols * m_numTargetRows, 1); // treat the values as one long vector
        }
        // layout case: reshape semantics happens across parallel seqeunces, i.e. requiring data shuffling
        else
        {
            if (weStack())
                Base::Unstack(fr, m_pMBLayout, Gradient(), InputPtr(0)->Gradient(), factor(), true /*addTo*/);
            else
                Base::Stack(fr.WithLayout(InputPtr(0)->GetMBLayout()), InputPtr(0)->GetMBLayout(), Gradient(), InputPtr(0)->Gradient(), factor(), true /*addTo*/);
        }
    }
...
private:
    ...
    bool weStack() const
    {
        return m_numTargetRows > InputPtr(0)->GetSampleMatrixNumRows();
    } // do we stack (multiple frames into one)
    size_t factor() const
    {
        return m_numTargetRows > InputPtr(0)->GetSampleMatrixNumRows() ? m_numTargetRows / InputPtr(0)->GetSampleMatrixNumRows() : InputPtr(0)->GetSampleMatrixNumRows() / m_numTargetRows;
