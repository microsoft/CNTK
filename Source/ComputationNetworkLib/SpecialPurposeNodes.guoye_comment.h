...
// guoye: RNNT(lmgraph, EncodeOutputLN, DecodeOutputLN,mysum_non,Wm, bm, 4125,delayConstraint=-1, tag="criterion"). So, InputRef(4)=Wm, InputRef(3)=mysum_non, InputRef(5)=bm, InputRef(0) is the input label sequence matrix, as: lmgraph=LabelsToGraph(lmin), and lmin=Input(ctclabledim,dynamicAxis=lminputaxis)

    // Compute gradients to input observations, the weights to the observations, and the class log posterior probabilities
    virtual void BackpropToNonLooping(size_t inputIndex) override
    {
        // Left node must be a scalar
        if (inputIndex == 0) //left derivative // guoye: the graident to LM input lmgraph is no need, so this one is a place holder
        {
            BackpropToLeft(*m_outputDensity, InputRef(inputIndex).Gradient(), Gradient());
        }
        else if (inputIndex == 1 || inputIndex == 2) //backprop to transcription f //  guoye: the gradient to F and G will be implemented in Plusbroadcast, so no use here
        {
            InputRef(inputIndex).Gradient().SetValue(0.0);
        }
        else if (inputIndex == 3) // guoye: the graident to mysum_non
        {
            BackpropToX(InputRef(inputIndex).Gradient(), Gradient(), *m_outputDensity, InputRef(4).Value());
        }
        else if (inputIndex == 5) // guyoe: the graident to bm
        {
            BackpropToB(InputRef(inputIndex).Gradient(), Gradient(), *m_outputDensity);
        }
        else if (inputIndex == 4) // guoye: the gradient to Wm
        {
  ...
  // guoye: graidentValues is the graident of objective function with respective to this node: see my onenote of "CNTK code logic" for more details. However, sicne the RNNTNode is the top Node, so the gradientValues will not be used. For the node that has parent node, the graidentValue has to be multiplied with the gradient value computed in the below function.
                     ...
                     Matrix<ElemType>& RNNTDerivative)
    {
        ...
        Matrix<ElemType>::VectorSum(RNNTDerivative, inputGradientValues, false); // guoye: colwise is false, see function of CPUMatrixImpl.h for defination: assume size of RNNTDerivative is M*N, inputGraidentValue is defined as M*1 vector, where inputGraidentValues[i] is the sum of all the elments in ith row. 
                     ...
                     Matrix<ElemType>& RNNTDerivative, Matrix<ElemType>& inputValue)
    {
		...
		//guoye: inputValue is mysum_non, here the inputGraidentValues is equal to RNNTDerivative * inputValue
		//  guoye: [this]=a*b, Matrix<ElemType>& Matrix<ElemType>::AssignProductOf(const Matrix<ElemType>& a, const bool transposeA, const Matrix<ElemType>& b, const bool transposeB)
    ...
    virtual void ForwardPropNonLooping() override
    {
        ...
        FrameRange fr(InputRef(0).GetMBLayout());
        InputRef(0).ValueFor(fr).VectorMax(*m_maxIndexes, *m_maxValues, true);
        // guoye: m_maxIndexes is in CNTK MB layout
        // compute CTC score
        ...
        size_t blankTokenId)
		*/
		// guoye: InputRef(1).Value() is F, i.e., EncodeOutputLN, InputRef(2).Value() is G, i.e., DecodeOutputLN, m_outputDensity stores the log posterior prob in u,t,k format (after the call of this function, m_outputDensity stores the gradient). m_derivative is not used in the future
