...
protected:
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        ...
        if (m_reluEng != nullptr && !result.GetSOB().IsView())
        {
    ...
    virtual void /*ComputationNode::*/ BackpropTo(const size_t inputIndex, const FrameRange& fr) override
    {
        ...
        if (m_reluEng != nullptr && !sliceInputGrad.GetSOB().IsView())
        {
            m_reluEng->Backward(sliceValue.GetSOB(), sliceOutputGrad.GetSOB(), sliceInputGrad.GetSOB());
        }
        else
        {
            sliceInputGrad.DoBinaryOpOf(Input(inputIndex)->IsGradientInitializedBy(this) ? 0.0f : 1.0f, sliceOutputGrad,
                                        sliceValue, 1, opElementwiseProductWithLinearRectifierDerivativeFromOutput,
                                        opSum);
        }
    }
    ...
    {
        return (binaryWithOutputGradient != noGradient) ? ParentGradientOptimization::Overwrite
                                                        : ParentGradientOptimization::None;
    }
...
public:
    ...
    RectifiedLinearNodeV2(int deviceId, const wstring& RectifiedLinear)
        : Base(deviceId, RectifiedLinear), m_reluEng(nullptr)
    {
    }
