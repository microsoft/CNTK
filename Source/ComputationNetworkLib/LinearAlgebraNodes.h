...
// BiVfsmnNode(hidden, bfilter, ffilter)
...
public:
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        ...
        // BEGIN DEBUG
        /*
        std::cout << Input(0)->Value().GetNumRows() << " " << Input(0)->Value().GetNumCols() << std::endl; // 2048 x 240
        std::cout << Input(1)->Value().GetNumRows() << " " << Input(1)->Value().GetNumCols() << std::endl; //  240 x 900
        std::cout << Value().GetNumRows() << " " << Value().GetNumCols() << std::endl;                     // 2048 x 900
        auto in1MBLayout = InputRef(1).GetMBLayout();
        auto in1Sequences = in1MBLayout->GetAllSequences();
        for (size_t i = 0; i < in1Sequences.size(); ++i)
        {
            let& in1Seq = in1Sequences[i];
            std::cout << in1Seq.seqId << " " << in1Seq.s << " " << in1Seq.tBegin << " " << in1Seq.tEnd << std::endl;
        }
        std::cout << in1MBLayout->GetNumTimeSteps() << " " << in1MBLayout->GetNumParallelSequences() << std::endl;
        */
        // END DEBUG
