...
public:
    ...
    virtual void /*ComputationNodeNonLooping::*/ ForwardPropNonLooping() override // -sum(left_i * log(softmax_i(right)))
    {
        ...
        */
        ...
        // set each frame to alinment and blank
        // eg, input frames: aaabbcc,
        //     labels_expended: aaaa aaaa aaaa bbbb bbbb cccc cccc
        // ==========================================
        // get crossbonding labels matrix at frist
        // labels_expended->Resize(InputRef(1).Value());
        size_t total_items = 0;
...
/*
        ...
        // set each frame to alinment and ##ZERO##
        // ZERO is to cancel the CE computation of this node
        // blank rate is very low
        // set blank at the last frame of a continues same unit
        // eg, input frames: aaabbcc,
        //     labels_expended: a000 a000 a-00 0b00 0b-0 00c0 00c-
        // ==========================================
        // get crossbonding labels matrix at frist
        // labels_expended->Resize(InputRef(1).Value());
        size_t total_items = 0;
        ...
        for (size_t s = 0; s < utt_frame2chanIdx.size(); ++s) // loop for each utt
        {
            ...
            for (size_t t = 0; t < num_frames; ++t)
            {
                ...
                for (size_t u = 0; u < num_units; ++u)
                {
                    ...
                    else if ((u == units_cnt + 1 && labels_max_values->GetValue(0, next_t_idx) == 2) || (t == num_frames - 1 && u == num_units - 1))
                    {
                        // set blank
                        size_t col_idx = u + t * num_units + total_items;
                        auto row_idx = labels_dim - 1;
...
*/
        ...
        // set each frame to alinment and ##ZERO##
        // ZERO is to cancel the CE computation of this node
        // blank rate is 0 if no blank in labels
        // eg, input frames: aaabbcc,
        //     labels_expended: a000 a000 a000 0b00 0b00 00c0 00c0
        // ==========================================
        // get crossbonding labels matrix at frist
        // labels_expended->Resize(InputRef(1).Value());
        size_t total_items = 0;
    ...
    virtual void /*ComputationNodeBase::*/
    Validate(bool isFinalValidationPass) override
    {
        ValidateBinaryReduce(isFinalValidationPass);
    }
