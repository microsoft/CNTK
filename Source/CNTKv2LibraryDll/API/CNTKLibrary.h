...
namespace CNTK
{
    ...
    public:
        ...
        ///
        CNTK_API NDArrayView(DataType dataType, const NDShape& viewShape, void* dataBuffer, size_t bufferSizeInBytes, const DeviceDescriptor& device, bool readOnly = false);

        /// Construct a read-only NDArrayView with the specified 'dataBuffer' as the backing storage.
        /// The 'dataBuffer' must have been allocated on the specified 'device', must be at least
        /// as large as the total size of the specified 'viewShape' and must outlive the created NDArrayView object.
        ///
        NDArrayView(DataType dataType, const NDShape& viewShape, const void* dataBuffer, size_t bufferSizeInBytes, const DeviceDescriptor& device)
        ...
        ///
        CNTK_API NDArrayView(DataType dataType, StorageFormat storageType, const NDShape& viewShape, const DeviceDescriptor& device);
    ...
    private:
        CNTK_API NDArrayView(DataType dataType, const DeviceDescriptor& device, StorageFormat storageType, const NDShape& viewShape, bool readOnly, void* tensorView);
    ...
    private:
        ...
        struct VariableFields final : public std::enable_shared_from_this<VariableFields>
        {
            ...
            VariableFields(const NDShape& shape, VariableKind varType, DataType type, Function* ownerFunction, const NDArrayViewPtr& value, bool needsGradient, const std::vector<Axis>& dynamicAxes, bool isSparse, const std::wstring& name)
