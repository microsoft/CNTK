...
#include "ParameterServerDistributedCommunicator.h"
...
namespace CNTK
{
    ...
    ParameterServerBasedCommunicatorPtr ParameterServerBasedCommunicator()
    {
        return std::make_shared<ParameterServerBasedCommunicatorImpl>();
    }

    // TODO deleting ASGDHelper too.
    void ParameterServerBasedCommunicator::Finalize() 
    {
    ...
    ParameterServerBasedCommunicatorImpl::ParameterServerBasedCommunicatorImpl()
    {
        ...
        m_asgdHelper.reset(NewASGDHelperV2<float>(m_mpi->NumNodesInUse()));
    }

    void ParameterServerBasedCommunicatorImpl::Initialize(const std::vector<NDArrayViewPtr>& values)
    {
    ...
    const std::unordered_set<DistributedWorkerDescriptor>& ParameterServerBasedCommunicatorImpl::Workers() const
    {
        return m_workers;
    }

    const DistributedWorkerDescriptor& ParameterServerBasedCommunicatorImpl::CurrentWorker() const
    {
        return m_currentWorker;
    }

    void ParameterServerBasedCommunicatorImpl::CheckWorkers(const std::unordered_set<DistributedWorkerDescriptor>& sendToWorkers)
    {
    ...
    void ParameterServerBasedCommunicatorImpl::Aggregate(const std::vector<NDArrayViewPtr>& values,
    ...
    DistributedCommunicatorPtr ParameterServerBasedCommunicatorImpl::SubGroup(const std::unordered_set<DistributedWorkerDescriptor>&) const
    {
        NOT_IMPLEMENTED;
    }

    void ParameterServerBasedCommunicatorImpl::Concatenate(const std::vector<ValuePtr>&, std::vector<ValuePtr>&, const std::unordered_set<DistributedWorkerDescriptor>&)
    {
        NOT_IMPLEMENTED;
    }

    void ParameterServerBasedCommunicatorImpl::Gather(
    ...
    void ParameterServerBasedCommunicatorImpl::Concatenate(const std::vector<NDArrayViewPtr>& input, std::vector<NDArrayViewPtr>& output, const std::unordered_set<DistributedWorkerDescriptor>& workers)
    {
    ...
    void ParameterServerBasedCommunicatorImpl::AggregateInPlace(
    ...
    void  ParameterServerBasedCommunicatorImpl::AggregateImpl(
    ...
    void  ParameterServerBasedCommunicatorImpl::Barrier()
    {
        m_mpi->WaitAll();
    }
