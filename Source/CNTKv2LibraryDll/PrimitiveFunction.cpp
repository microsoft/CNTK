...
namespace CNTK
{
    ...
    /*static*/ const std::wstring PrimitiveFunction::AttributeRenameOutputs = L"RenameOutputs";
    ...
    void PrimitiveFunction::InferOutputs(std::vector<Variable>& outputs)
    {
        ...
        {
            bool renameOutputs = false;
            if (m_attributes.Contains(PrimitiveFunction::AttributeRenameOutputs))
                renameOutputs = m_attributes[PrimitiveFunction::AttributeRenameOutputs].Value<bool>();
            ...
            for (auto input : m_inputs)
            {
                ...
                if (renameOutputs)
                {
                    outputName = Name();
                    if (!outputs.empty())
                        outputName += L"_" + std::to_wstring(outputs.size());
                }
                else
                    outputName = input.Name();

                outputs.push_back(OutputVariable(input.Shape(), input.GetDataType(), input.DynamicAxes(), outputName));
            }
        }
        else
        {
                                                          ...
                                                          const CNTK::DeviceDescriptor& device)
    {
        ...
        // new op type values are only added to the end of the list, after EditDistanceError.
