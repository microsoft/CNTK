...
private:
    ...
    Variable ResolveFunction(const ComputationNodeBasePtr& node, std::vector<Variable>& inputVars)
    {
        ...
        else if (node->OperationName() == OperationNameOf(TimeReductionNode))
        {
            ...
            primitiveFunctionConfigParameters[PrimitiveFunctionAttribute::AttributeNameReductionFactor] = timeReductionNode->GetReductionFactor();
            
            opType = PrimitiveOpType::TimeReduction;
        }
