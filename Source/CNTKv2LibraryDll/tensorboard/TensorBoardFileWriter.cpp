...
namespace CNTK
{
    ...
    {
        template<typename T>
        void Encode(char* buf, T value)
        {
            // Assume little-endian target encoding.
            memcpy(buf, &value, sizeof(value));
        }

        uint32_t GetMaskedCrc(const char* data, size_t n)
        {
            // Compute CRC32C.
        ...
        std::wstring GetNewFilePath(const std::wstring& dir, time_t time)
        {
        ...
        std::string Serialize(const tensorflow::Event& event)
        {
            std::string record;
    ...
    bool TensorBoardFileWriter::Close()
    {
        ...
        if (fclose(m_file))
        {
            fprintf(stderr, "TensorBoardFileWriter: Error closing previous the event file (%ls).", m_fileName.c_str());
            success = false;
        }
