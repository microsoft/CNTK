...
private:
    ...
    public:
        ...
        //
        // ONNX model description (of CNTK exported model) is in this format:
        // <<<ONNXModelOutputName, CNTKModelOutputName>>> pair: <<<onnx_name_0, cntk_name_0>>> <<<onnx_name_0, cntk_name_1>>>
        //
...
private:
    ...
    static NodeArg* GetInputAdjustmentForBroadcast(onnxruntime::Graph* graph, const FunctionPtr src, const Variable &input, int inputIndex,
                                                   ...
                                                   const std::unordered_map<Variable, Variable>& compositeOutputsMap,
                                                   const std::string& scanInputName = "");
                                     ...
                                     std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    for (size_t inputIndex = 0; inputIndex < src->Inputs().size(); ++inputIndex)
    {
        ...
        if ((isOutputOfStepFunction && isInSubGraph) || isScanInputInSubgraph)
        {
            ...
            adjusted = GetInputAdjustmentForBroadcast(graph, src, input, inputIndex, inputArgType, compositeOutputsMap, inputName);
        }
        else
        {
            adjusted = GetInputAdjustmentForBroadcast(graph, src, input, inputIndex, inputArgType, compositeOutputsMap);
        }
                                                   ...
                                                   bool isSequenceReduceElement)
{
    ...
    if ((reductionAxes.size() == 1 && (reductionAxes[0] == Axis::DefaultBatchAxis() ||
        ...
        reductionAxes[0] == Axis::AllAxes()))) {    // Reduction on batch axis in CNTK removes the batch axis, even if keepdims is true.
