...
private:
    ...
    static onnxruntime::Node* CreateTimesNode(const FunctionPtr &src,
        onnxruntime::Graph* graph,
        std::unordered_map<FunctionPtr, onnxruntime::Node*>& functionNodes,
        std::unordered_map<Variable, onnxruntime::Node*>& variableNodes,
        const std::unordered_map<Variable, Variable>& compositeOutputsMap,
    ...
    static Node* CreateNodesFromTimes(const std::string nodeName,
        onnxruntime::Graph* graph, const std::vector<onnxruntime::NodeArg *>& orderedInputs,
        const std::vector<onnxruntime::NodeArg *>& outputs, size_t py_api_output_rank_argument,
        int numDynamicAxes1, int numDynamicAxes2, CNTK::DataType dataType);
...
void CNTKToONNXHelper::CopyTensor(const NDArrayViewPtr src, onnx::TensorProto& dst, onnx::TypeProto *inputArgType /*=nullptr*/)
{
    ...
    {
        CopyShapeTypeProtoToTensorProto(*inputArgType, dst);
        //for (int dim = 0; dim < dst.dims_size(); dim++)
        //{
        //    // onnx::TensorProto is to be added to initializer. it shall not have any symbolic dimension.
        //    // 
        //    if (dst.dims(dim) <= 0)
        //        (*dst.mutable_dims())[dim] = (int64_t)1;
        //}
    }
    else
    {
...
onnx::TypeProto CNTKToONNXHelper::ToTypeProto(const NDShape& shape, int dynamicAxisCount)
{
    ...
    for (int i = 0; i < dynamicAxisCount; i++)
    {
        ...
        {
            // batch axis
            newShape.mutable_tensor_type()->mutable_shape()->add_dim()->set_dim_param(FreeBatchDimParam);
        }
...
onnx::TypeProto CNTKToONNXHelper::ToTypeProto(const NDShape& shape, bool hasBatchAxis, bool hasSequenceAxis, bool doReverseShape)
{
    ...
    for (auto dimension : dimensions)
    {
        ...
        else if (dimension == FreeBatchDimension)
        {
            newShape.mutable_tensor_type()->mutable_shape()->add_dim()->set_dim_param(FreeBatchDimParam);
        }
        else
...
std::vector<int64_t> CNTKToONNXHelper::ToINTS(const onnx::TypeProto& shape)
{
    ...
    for (int i = 0; i < shape.tensor_type().shape().dim_size(); i++)
    {
        ...
        else if (shape.tensor_type().shape().dim(i).dim_param() == FreeBatchDimParam)
        {
            newShape.push_back(FreeBatchDimension);
        }
        else // sequence axis
        {
    ...
    const std::string &uid, std::vector<onnxruntime::NodeArg *> &nodeInputs)
{
    ...
    {
        std::vector<int64_t> shape({ (int64_t)initialVariables.size(), 1, cellSize });
        ...
        if (batchSize != 1)
        {
            shape[1] = FreeBatchDimension;
            Node* expandBatchNode = AddExpandNode(inputArg, shape, inputArg.Name() + "_expand_batch_output", graph);
            nodeInputs.push_back(const_cast<NodeArg *>(expandBatchNode->OutputDefs()[0]));
        }
        else
            nodeInputs.push_back(&inputArg);
    }
    else
    {
        ...
        if (initialVariables[0].Owner())
        {
            Node *initialiStateNode = CreateNode(initialVariables[0].Owner(),
                graph, functionNodes, variableNodes, compositeOutputsMap, scanLoops, createLoopIndex);
                                                    ...
                                                    std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
...

    {
        ...
        if (has_initial_h)
        {
            ...
            PrepareLSTMInitialStateNode(graph, functionNodes, variableNodes, compositeOutputsMap, scanLoops, createLoopIndex, 
                initialHs, FreeBatchDimension, hidden_size, hiddenUid, nodeInputs);
        }
        else
        {
            AddEmptyInput(graph, nodeInputs);
        ...
        if (has_initial_c)
        {
            ...
            PrepareLSTMInitialStateNode(graph, functionNodes, variableNodes, compositeOutputsMap, scanLoops, createLoopIndex, 
                initialCs, FreeBatchDimension, hidden_size, cellUid, nodeInputs);
        }
        else
        {
            AddEmptyInput(graph, nodeInputs);
...

    {
        auto outputArgType = ToTypeProto(std::vector<int64_t>({ (int64_t)NDShape::FreeDimension, 
            (int64_t)Yhs.size(), FreeBatchDimension, (int64_t)Yhs[0].Shape()[0]}), false);
    ...
    std::vector<int64_t> shape({ (int64_t)NDShape::FreeDimension, FreeBatchDimension, hidden_size });
...
void AddShapeInitializer(const std::string& shapeInputArgName, const std::vector<int64_t> &newShape, Graph *graph)
{
    ...
    for (size_t index = 0; index < newShape.size(); index++)
        // ONNX dimensions are left aligned. this means sequence and batch, if reshaped to ReshapeKeepInputDim,
        // it will be kept as the same from input.
        if (newShape[index] == NDShape::FreeDimension || newShape[index] == FreeBatchDimension)
        {
                                                                     ...
                                                                     const std::string &nodeOutputName)
{
    ...
    // Transpose batch and direction axes

    std::vector<int64_t> perm(node->OutputDefs()[0]->Shape()->dim_size());
    std::generate(perm.begin(), perm.end(), [axis = 0]() mutable { return axis++; });
    std::swap(perm[1], perm[2]);

    onnx::TypeProto transposeOutputArgType = *node->OutputDefs()[0]->TypeAsProto();
    onnx::TensorShapeProto_Dimension dimTemp = transposeOutputArgType.tensor_type().shape().dim(2);
    *transposeOutputArgType.mutable_tensor_type()->mutable_shape()->mutable_dim(2) =
        transposeOutputArgType.tensor_type().shape().dim(1);
    *transposeOutputArgType.mutable_tensor_type()->mutable_shape()->mutable_dim(1) = dimTemp;
    NodeArg* transposeOutputNodeArg = &graph->GetOrCreateNodeArg(node->OutputDefs()[0]->Name() + "_transpose", &transposeOutputArgType);

    Node* transposeNode = &graph->AddNode(node->OutputDefs()[0]->Name() + "_transpose", "Transpose", "",
        {const_cast<NodeArg *>(node->OutputDefs()[0])}, { transposeOutputNodeArg });
    transposeNode->AddAttribute("perm", perm);
    ...
    auto outputArgs = transposeNode->OutputDefs();
                                                               ...
                                                               std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    if (src->OpName() == L"Delay")
    {
        ...
        {
            Node* delayNode = CreateNode(src->BlockRoot(), graph, functionNodes, variableNodes, compositeOutputsMap,
                                                                     ...
                                                                     bool isFirst)
{
    ...
    for (int n = 2; n < src->Inputs()[2].Shape().Rank() + 2; n++)
    {
                                                                 ...
                                                                 Variable input, NodeArg* inputNodeArg, const std::string &nodeName, bool inLoop)
{
    ...
    if (!inLoop || !ScanWithoutBatchAxis)
        shapeProto.add_dim()->set_dim_param(FreeBatchDimParam);
                                                             ...
                                                             std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    if (seq_dim_is_1)
    {
        return AddSqueezeNode(outputNodeArg, std::vector<int64_t>({ 0 }), outputName, graph);
    }
    else
            ...
            if (OutputOfLoop(scanLoops, src, loopIndex) && !scanLoops[loopIndex].m_scanOpCreated)
            {
                ...
                for (auto &scanLoopState : scanLoops[loopIndex].scanLoopStates)
                {
                                                     ...
                                                     *scanInitialStateNodeArg->Shape()))
                    {
                        ...
                        {
                            TypeProto batchIsOneTypeProto = calculatedScanInitialStateTypeProto;
                            batchIsOneTypeProto.mutable_tensor_type()->mutable_shape()->mutable_dim(0)->set_dim_value(1);
                            scanInitialStateNodeArg = &graph->GetOrCreateNodeArg(
                                scanInitialStateNodeArgName, &batchIsOneTypeProto);

                            const std::vector<int64_t> shape = ToINTS(calculatedScanInitialStateTypeProto);
                            Node* expandBatchNode = AddExpandNode(*scanInitialStateNodeArg, shape,
                                scanInitialStateNodeArg->Name() + "_expand_batch_output", graph);
                            input_args.push_back(const_cast<NodeArg *>(expandBatchNode->OutputDefs()[0]));
                        }
                        else
                        {
                            scanInitialStateNodeArg = &graph->GetOrCreateNodeArg(
    ...
    std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    else if (cntkOpName == "ToSequenceLikeOp")
    {
    ...
    {
        return CreateConvolutionBlockNode(src, graph, functionNodes, variableNodes, compositeOutputsMap, scanLoops, createLoopIndex);
    }
    ...
    for (int i = 0; i < inputs.size(); i++)
    {
        if (inputs[i]->Shape()->dim_size() == outputRank)
        {
            if (inputs[i]->Shape()->dim_size() > batchAxisIndexInOnnx &&
                inputs[i]->Shape()->dim(batchAxisIndexInOnnx).dim_value() != FreeBatchDimension)
            {
                                                                 ...
                                                                 std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    bool inSubgraph = createLoopIndex >= 0;
    {
        ...
        onnx::TypeProto initialStateInputArgType = ToTypeProto(stateInput.Shape(), 
            stateInput.HasBatchAxis(), 
            stateInput.HasSequenceAxis());
        // batch axis is symbolic which cannot be put into an initializer. we use batch=1 and expand later
        // initialStateInputArgType.mutable_tensor_type()->mutable_shape()->mutable_dim(0)->set_dim_value(1);
        ...
        if (inSubgraph)
        {
            ...
            if (isInitialStateConstant)
            {
                ...
                if (ScanWithoutBatchAxis)
                    initialStateShape.insert(initialStateShape.begin(), FreeBatchDimension);

                // set batch axis to 1. It will be expanded when adding to scan input.
                initialStateShape[0] = 1;
                                     ...
                                     std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    for (size_t inputIndex = 0; inputIndex < src->Inputs().size(); ++inputIndex)
    {
        ...
        if (cntkOpName == "Splice")
        {
            ...
            inputArgType = ToTypeProto(input.Shape(), !isConstant && OpInputsHasBatchAxis(src), !isConstant && input.HasSequenceAxis());
        }
        ...
        if (cntkOpName == "Reshape")
        {
            ...
            if (src->Output().HasBatchAxis())
                newShapeVec.push_back(FreeBatchDimension);
                ...
                if ((inputNodeArg->Shape()->dim((int)axisIndex1).dim_param() == FreeSequenceDimParam &&
                    ...
                    (inputNodeArg->Shape()->dim((int)axisIndex2).dim_param() == FreeSequenceDimParam &&
                     axisIndex2 == 0 && axisIndex1 != 1) &&
                        inputNodeArg->Shape()->dim(1).dim_param() == FreeBatchDimParam)
                {
    ...
    for (int i = 0; i < shapes.size(); i++)
    {
        ...
        for (int index_to_shape_i = 0; index_to_shape_i < shape_i.size(); index_to_shape_i++)
        {
            ...
            if (ignoreAxes.find(onnx_axis) != ignoreAxes.end())
                ...
                continue;
            else if (broadcast_shape[onnx_axis] == 1 && shape_i[index_to_shape_i] != 0)
                broadcast_shape[onnx_axis] = shape_i[index_to_shape_i];
            else if (broadcast_shape[onnx_axis] != shape_i[index_to_shape_i] && 
                (shape_i[index_to_shape_i] != 1 && shape_i[index_to_shape_i] != 0))
...
onnxruntime::Node* CNTKToONNXHelper::AddNode(const FunctionPtr& src, onnxruntime::Graph* graph, const std::vector<onnxruntime::NodeArg *>& inputs, const std::vector<onnxruntime::NodeArg *>& outputs)
{
    ...
    {
        if (src->OpName() == L"LayerNormalization")
        {
                                                                         ...
                                                                         std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    std::vector<int64_t> transposeOutputShape({ FreeBatchDimension, (int64_t)NDShape::FreeDimension, (int64_t)numDirections, hidden});
    ...
    newShape.insert(newShape.begin(), FreeBatchDimension);
                                                               ...
                                                               size_t numDirections, size_t hiddenSize, CNTK::DataType outputType, string adapterBasename)
{
    ...
    std::vector<int64_t> shape({(int64_t)NDShape::FreeDimension, (int64_t)NDShape::FreeDimension, (int64_t)(numDirections * hiddenSize)});
                                                              ...
                                                              std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    else
    {
        ...
        if (src->Inputs()[0].HasSequenceAxis())
        {
            ...
            finalOutputShape[1] = FreeBatchDimension;
...
//    Variable rightInput = src->Inputs()[0], leftInput = src->Inputs()[1];
//    int rightInputRank = rightInput.Shape().Rank();
//    int rightHasBatchAxis = rightInput.HasBatchAxis();
//    int rightHasSequenceAxis = rightInput.HasSequenceAxis();
//    if (rightInputRank == 2)
//    {
//        // need transpose
//        TypeProto transposeOutputTypeProto = *inputs[0]->TypeAsProto(); 
//        ::google::protobuf::RepeatedPtrField< ::onnx::TensorShapeProto_Dimension >* mutable_dim =
//            transposeOutputTypeProto.mutable_tensor_type()->mutable_shape()->mutable_dim();
//        std::vector<int> perm;
//        if (rightHasBatchAxis)
//        {
//            if (rightHasSequenceAxis)
//            {
//                perm = std::vector<int>({ 0, 1, 3, 2 });
//                std::swap((*mutable_dim)[2], (*mutable_dim)[3]);
//            }
//            else
//            {
//                perm = std::vector<int>({ 0, 2, 1 });
//                std::swap((*mutable_dim)[1], (*mutable_dim)[2]);
//            }
//        }
//        else
//        {
//            perm = std::vector<int>({ 1, 0 });
//            std::swap((*mutable_dim)[0], (*mutable_dim)[1]);
//        }
//
//        onnxruntime::NodeArg &transposeOutputArg = graph->GetOrCreateNodeArg(
//            UniqueNodeNameStorage::GetUniqueNodeNameWithoutUid(nodeName + "_transpose_out"), &transposeOutputTypeProto);
//        onnxruntime::Node* transposeNode = &graph->AddNode(
//            UniqueNodeNameStorage::GetUniqueNodeNameWithoutUid(nodeName + "_transpose"),
//            "Transpose", "", { inputs[0] }, { &transposeOutputArg });
//        transposeNode->AddAttribute("perm", ToINTS(perm, false));
...
//        // size_t py_api_output_rank_argument = 2; 
//        
...
//        auto nodeName = UniqueNodeNameStorage::GetUniqueNodeName(src);
//
//        return CreateNodesFromTimes(nodeName, graph, { inputs[1], &transposeOutputArg }, { &matmulOutputArg },
//            py_api_output_rank_argument, numDynamicAxes1, numDynamicAxes2, src->Inputs()[1].GetDataType());
//    }
//    else // rightInputRank == 1
//        if (rightHasBatchAxis)
//        {
//            fprintf(stderr, "times_tranpose with left tensor as a vector input is not supported for CNTK/ONNX exporter.");
//
//            // still create a reasonable node which may not match ONNX spec.
//            size_t py_api_output_rank_argument = src->Attributes()[L"outputRank"].Value<size_t>();
//            // CNTK Times swaps input order
//            int numDynamicAxes1 = src->Inputs()[1].DynamicAxes().size();
//            int numDynamicAxes2 = src->Inputs()[0].DynamicAxes().size();
//            auto nodeName = UniqueNodeNameStorage::GetUniqueNodeName(src);
//
//            return CreateNodesFromTimes(nodeName, graph, { inputs[1], inputs[0] }, { outputs[0] },
//                py_api_output_rank_argument, numDynamicAxes1, numDynamicAxes2, src->Inputs()[1].GetDataType());
//        }
//        else // right value is a 1d vector
//        {
//            onnxruntime::NodeArg &matmulOutputArg = graph->GetOrCreateNodeArg(outputs[0]->Name(), nullptr);
//            onnxruntime::Node* matmulNode = &graph->AddNode(UniqueNodeNameStorage::GetUniqueNodeNameWithoutUid(nodeName + "_matmul"),
//                "MatMul", "", { inputs[1], inputs[1] }, { &matmulOutputArg });
//
//            functionNodes.emplace(src, matmulNode);
//            return matmulNode;
//        }
                                                               ...
                                                               std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    auto functionNode = [&]() -> onnxruntime::Node* {
        ...
        {
            // CNTK: [*,#][d0,d1,...,dn]->[*,#][d0 x d1 x d_k-1,d_kx x...,dn]
            // ONNX: [*,#,d0,d1,...,dn]->[* x # x d0 x d1...d_k-1,d_kx...dn]->[*,#,d0 x d1 x d_k-1,d_k x...,dn]
            // so we do: reshape (newShape = [*,#,d0 x d1 x d_k-1,d_k x...,dn]

            onnx::TypeProto outputReshapeOut = ToTypeProto(flattenOutput.Shape(), 
                flattenInput.HasBatchAxis(), flattenInput.HasSequenceAxis());
            UpdateONNXType(flattenOutput.GetDataType(), outputReshapeOut);

            onnxruntime::Node* reshapeAsFlattenNode = AddReshapeNodeImpl(graph, 
                UniqueNodeNameStorage::GetUniqueNodeNameWithoutUid(nodeName + "_reshape_as_flatten"), 
                inputs[0], outputs[0], ToINTS(outputReshapeOut));

            return reshapeAsFlattenNode;
        }
        else
        {
    ...
    std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    {
        onnxruntime::Node* functionNode = CreateNode(src->BlockRoot(), graph, functionNodes, variableNodes, compositeOutputsMap,
            scanLoops, createLoopIndex);
...
onnxruntime::Node* CNTKToONNXHelper::CreateTimesNode(const FunctionPtr &src,
    onnxruntime::Graph* graph,
    std::unordered_map<FunctionPtr, onnxruntime::Node*>& functionNodes,
    std::unordered_map<Variable, onnxruntime::Node*>& variableNodes,
    const std::unordered_map<Variable, Variable>& compositeOutputsMap,
    std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    int numDynamicAxes2 = src->Inputs()[0].DynamicAxes().size();
    auto nodeName = UniqueNodeNameStorage::GetUniqueNodeName(src);
    return CreateNodesFromTimes(nodeName, graph, orderedInputs, outputs, py_api_output_rank_argument,
        numDynamicAxes1, numDynamicAxes2, src->Inputs()[1].GetDataType());
}

Node* CNTKToONNXHelper::CreateNodesFromTimes(const std::string nodeName,
    onnxruntime::Graph* graph, const std::vector<onnxruntime::NodeArg *>& orderedInputs,
    const std::vector<onnxruntime::NodeArg *>& outputs, size_t py_api_output_rank_argument,
    int numDynamicAxes1, int numDynamicAxes2, CNTK::DataType dataType)
{
    ...
    if (reductionRank > 1 || py_api_output_rank_argument > 1) // We need to insert reshape.
    {
        ...
        // CNTK Times swaps input order
        UpdateONNXType(dataType, matMulInput1Reshape);
        UpdateONNXType(dataType, matMulInput2Reshape);
        UpdateONNXType(dataType, matMulOutputShape);
        ...
        if (py_api_output_rank_argument > 1)
        {
            ...
            std::vector<int64_t> finalOutputShape = ToINTS(*outputs[0]->TypeAsProto());
            Node* node = AddReshapeNodeImpl(graph, nodeName + "_output_reshape", &matMulOutputNodeArg, outputs[0], finalOutputShape);
            return node;
        }
        else
        {
            Node* node = &graph->AddNode(nodeName, "MatMul", "", { &inputOutput1Arg, &inputOutput2Arg }, outputs);
            return node;
        }
    }
    else
    {
        Node* node = &graph->AddNode(nodeName, "MatMul", "", orderedInputs, outputs);
        return node;
    }
}
