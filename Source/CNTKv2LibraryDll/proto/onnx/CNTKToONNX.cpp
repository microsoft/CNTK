...
private:
    ...
    static onnxruntime::Node* CreateUnpackSequenceNode(const FunctionPtr& src,
        ...
        const std::unordered_map<Variable, Variable>& compositeOutputsMap,
        std::vector<ScanLoop> &scanLoops, int createLoopIndex);
    static onnxruntime::Node* CreateSequenceWhereNode(const FunctionPtr& src,
        onnxruntime::Graph* graph,
        std::unordered_map<FunctionPtr, onnxruntime::Node*>& functionNodes,
        std::unordered_map<Variable, onnxruntime::Node*>& variableNodes,
        const std::unordered_map<Variable, Variable>& compositeOutputsMap,
...
int64_t CNTKToONNXHelper::ConvertAxisToOnnx(const Axis &axis, const Variable &operand)
{
    ...
    if (CNTKToONNXHelper::isProcessingScan && operand.DynamicAxes().size() == 2)
    {
        ax--;
        if (ScanWithoutBatchAxis)
            ax--;
...
onnxruntime::Node* CNTKToONNXHelper::CreateSequenceWhereNode(const FunctionPtr& src,
    onnxruntime::Graph* graph,
    std::unordered_map<FunctionPtr, onnxruntime::Node*>& functionNodes,
    std::unordered_map<Variable, onnxruntime::Node*>& variableNodes,
    const std::unordered_map<Variable, Variable>& compositeOutputsMap,
    std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    Node* whereNode = graph->AddNode(UniqueNodeNameStorage::GetUniqueNodeName(src), "Where", "",
        inputs, outputs);
    ...
    if (!scanLoops.empty())
    {
        ...
        else
        {
            ...
            if (OutputOfLoop(scanLoops, src, loopIndex) && !scanLoops[loopIndex].m_scanOpCreated)
            {
                ...
                scanNode->AddAttribute("scan_input_directions", directions);
                // CNTK input/output direction match
                ...
                scanNode->AddAttribute("handle_batch", (int64_t)(ScanWithoutBatchAxis ? 1 : 0));
...
onnxruntime::Node* CNTKToONNXHelper::CreateNode(const FunctionPtr& src,
    ...
    std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    std::cout << ToLegacyString(ToUTF8(src->Name())) << " : " << ToLegacyString(ToUTF8(src->Uid()))
        << ": " << ToLegacyString(ToUTF8(src->OpName())) << std::endl;
    if (src->Uid() == L"Block332712")
        std::cout << "";
    ...
    if (cntkOpName == "Where")
    {
        ...
        return CreateSequenceWhereNode(src,
            graph,
            functionNodes,
            variableNodes,
            compositeOutputsMap,
            ...
            if (variableNodes.find(input) == variableNodes.end())
            {
                if (input.IsParameter() || input.IsConstant())
                {
                    ...
                    if (isInitialStateOfSubGraph)
                    {
                        ...
                        if (ScanWithoutBatchAxis)
                            initialStateShape.insert(initialStateShape.begin(), FreeBatchSize);
    ...
    std::vector<ScanLoop> &scanLoops, int createLoopIndex)
{
    ...
    for (size_t i = 0; i < numLayers; ++i)
    {
        ...
        auto outArgName = (i == numLayers - 1)
            ? UniqueNodeNameStorage::GetUniqueOutputNodeName(ornnOutput)
            : UniqueNodeNameStorage::GetUniqueNodeNameWithoutUid(ToLegacyString(ToUTF8(ornnOutput.Uid())) + "_" + std::to_string(i));
