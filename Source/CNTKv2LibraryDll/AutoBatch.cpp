...
class NDArrayViewArena : public NDArrayView::IAllocator
{
    ...
    NDArrayViewPtr NewSparse(const NDShape& shape, const DataType& dataType, StorageFormat storageFormat, const DeviceDescriptor& device)
    {
        ...
        fail_if(formatAsIndex >= s_recycledSparseArenass.size(), "unexpected storageFormat int value??");
        auto& s_recycledArenas = s_recycledSparseArenass[formatAsIndex]; // TODO: use only one set, since we check the type anyway
...
public:
    ...
    NewGradient CacheAndGetGradientView(const Variable& input, StorageFormat format = StorageFormat::Dense)
    {
        ...
        // that's our gradient
        // Note: No need to cache it because we only ever backprop into a function's input once
    ...
    void BackpropToUnbatched(const AutoBatchConsumer& fi, bool viewAllowed)
    {
        ...
        if (viewAllowed && IsOpGradientViewable(op, index, outputFields, input))
        {
            ...
            else
            {
                ...
                ReplaceWithReshapedViewIfNeeded(outputGradient, redirectedInputFields.m_shape);
                fail_if(redirectedInputFields.m_gradient, "function with viewable gradient unexpectedly already has a gradient??");
                redirectedInputFields.m_gradient = move(outputGradient); // and the redirected location. If it is the same, then this will do nothing.
                m_stats.numGradientsReferenced++;
            }
