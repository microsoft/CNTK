...
namespace CNTK
{
    ...
    FunctionPtr LatticeSequenceWithSoftmax(const Variable& labels, const Variable& prediction, const Variable& scaledLogLikelihood, const Variable& teacherLL, const Variable& lattice, const std::wstring& symListPath, const std::wstring& phonePath, const std::wstring& stateListPath, const std::wstring& transProbPath, const std::wstring& latticeConfigPath, float hSmoothingWeight, float frameDropThresh, bool doReferenceAlign, bool seqGammarUsesMBR, float seqGammarAMF, float seqGammarLMF, float seqGammarBMMIFactor, float seqGammarWordPen, bool TSLearn, const std::wstring& name)
    {
        auto additionalProperties = Dictionary();
        ...
        additionalProperties[PrimitiveFunctionAttribute::AttributeNameTSLearn] = TSLearn;
