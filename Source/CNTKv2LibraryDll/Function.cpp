...
namespace CNTK
{
    ...
    Function::Function(const Variable& input0, const Variable& input1, Dictionary&& functionConfig, const std::wstring& name)
        : m_inputs(input0, input1), m_name(name), m_attributes(std::move(functionConfig))
    {
    }
    ...
    Function::Function(const Variable& input0, Dictionary&& functionConfig, const std::wstring& name)
        : m_inputs(input0), m_name(name), m_attributes(std::move(functionConfig))
    {
    }
    ...
    void Function::Restore(const std::wstring& filepath)
    {
        ...
        auto loadedModelLeafVariables = loadedModelFunction->Inputs();
            ...
            // TODO: master has this: std::function<void(const std::unordered_set<FunctionPtr>&, std::map<std::wstring, Variable>&)> RemovePastAndFutureValueInitialStateScalarConstants;
    ...
    // TODO: Since this is Dynamite only, we could as well just return a Variable that owns the Primitive
    FunctionPtr BatchNormalization(const Variable& operand,
                                   size_t id,
                                   const Variable& scale,
                                   const Variable& bias,
    ...
    // TODO: This is Dynamit eonly, so we could as well return a Variable referencing the Primitive.
