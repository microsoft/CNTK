    ...
    void CheckOutputNodes(NDLScript<ElemType>* script, std::string symbolName, std::wstring groupTag)
    {
        ...
        if (nodeArray)
        {
            ...
            for (size_t i = 0; i < nodes.size(); i++)
            {
                ...
                if (!cnNode)
                    RuntimeError("Invalid node '%s' as a(n) %s node, nonexistant or wrong type", nodes[i]->GetName().c_str(), Microsoft::MSR::CNTK::ToLegacyString(Microsoft::MSR::CNTK::ToUTF8(groupTag)).c_str());
