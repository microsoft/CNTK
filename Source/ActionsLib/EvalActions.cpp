    ...
    if (config.Exists("writer"))
    {
       ...
       // ConfigParameters
        if (decodeType == 0)
            ...
            writer.WriteOutput_beam(testDataReader, mbSize[0], testDataWriter, outputNodeNamesVector, epochSize, writerUnittest, decodeBeam, decodeExpandBeam, reset, withEOS, indictfile,thresh);
            //writer.WriteOutput(testDataReader, mbSize[0], testDataWriter, outputNodeNamesVector, epochSize, writerUnittest);
