...
CNTKCORE_DEFS = os.path.abspath(
    os.path.join(DIRNAME_OF_THIS_FILE, os.pardir, 'CNTK.core.bs'))
...
"""
    ...
    def __init__(self, comp_match):
        ...
        if self.name in INPUT_NODES:
            self.parentclass = 'InputComputationNode'     
        elif self.name in IMAGE_INPUT_NODES:
            self.parentclass = 'ImageInputComputationNode'     
        else:
...
from cntk.graph import ComputationNode, InputComputationNode, ImageInputComputationNode
...
if __name__ == '__main__':
    convert_bs_to_python(CNTKCORE_DEFS, os.path.join(os.pardir, "ops.py"))
