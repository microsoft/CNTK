// <copyright file="BinaryReader.h" company="Microsoft">
...
struct ViewPosition
{
    ...
    size_t size;
    int refCount;
    ViewPosition():view(NULL),filePosition(0),size(0),refCount(0)
    {}
...
public:
    ...
    vector<ViewPosition>::iterator NotFound() {return m_views.end();}
    ...
    size_t GetViewAlignment() {return m_viewAlignment;}
    bool Writing() {return m_writeFile;}
    size_t GetFilePositionMax() {return m_filePositionMax;}
...
protected:
    ...
    bool CheckBounds(size_t index) {return index >= m_elemMin && index < m_elemMax;}
...
public:
    ...
    // element accessors
    template <typename T> T& operator [](size_t index) { return *(T*)GetElement(index); }
    template <typename T> T* GetElement(size_t index) const { return (T*)(GetElement(index)); }
    ...
    // accessors
    SectionFile* GetSectionFile() const {return m_file;}
    SectionHeader* GetHeader() const {return m_sectionHeader;}
    size_t GetFilePosition() const {return m_filePosition;}
    void SetFilePosition(size_t filePosition) {m_filePosition = filePosition;}
    ...
    size_t GetSize() const {return m_sectionHeader->size;}
    void SetSize(size_t size) {m_sectionHeader->size = size;}
    size_t GetSizeAll() const {return m_sectionHeader->sizeAll;}
    void SetSizeAll(size_t size) {m_sectionHeader->sizeAll = size;}
    size_t GetHeaderSize() const {return m_sectionHeader->sizeHeader;}
    ...
    void SetElementSize(size_t elementSize) {assert(elementSize < 0x10000);m_sectionHeader->bytesPerElement = (WORD)elementSize;}
    size_t GetElementSize() const {return (size_t)m_sectionHeader->bytesPerElement;}
    // ElementsPerRecord - number of elements per record
    void SetElementsPerRecord(size_t elementsPerRecord) {assert(elementsPerRecord < 0x10000); m_sectionHeader->elementsPerRecord = (WORD)elementsPerRecord;}
    size_t GetElementsPerRecord() const {return (size_t)m_sectionHeader->elementsPerRecord;}
    // ElementCount - number of total elements stored
    void SetElementCount(size_t elementCount) {m_sectionHeader->elementsCount = elementCount;}
    size_t GetElementCount() const {return m_sectionHeader->elementsCount;}
    ...
    Section* GetParent() const {return m_parent;}

    void SetName(const std::wstring& name) {m_name = name;}
    const std::wstring& GetName();
    int GetSectionCount() const {return m_sectionHeader->dataSections;}
    size_t GetRecordCount() const {return m_sectionHeader->elementsPerRecord!=0?m_sectionHeader->elementsCount/m_sectionHeader->elementsPerRecord:m_sectionHeader->elementsCount;}
...
public:
    ...
    virtual ~SectionFile();
    Section *FileSection() {return m_fileSection;}
