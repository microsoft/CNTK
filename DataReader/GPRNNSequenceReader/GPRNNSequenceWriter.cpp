//
// <copyright file="GPRNNSequenceWriter.cpp" company="Microsoft">
...
namespace Microsoft { namespace MSR { namespace CNTK {
    ...
    bool GPRNNSequenceWriter<ElemType>::compare_val(const ElemType& first, const ElemType& second)
    {
        return ( first < second );
    }

    template<class ElemType> template<class ConfigRecordType>
    void GPRNNSequenceWriter<ElemType>::InitFromConfig(const ConfigRecordType& writerConfig)
    {
        udims.clear();
    ...
    template<class ElemType>
    void GPRNNSequenceWriter<ElemType>::ReadLabelInfo(const wstring & vocfile, 
    ...
    template<class ElemType>
    void GPRNNSequenceWriter<ElemType>::Destroy()
    {
    ...
    template<class ElemType>
    bool GPRNNSequenceWriter<ElemType>::SaveData(size_t /*recordStart*/, const std::map<std::wstring, void*, nocase_compare>& matrices, size_t /*numRecords*/, size_t /*datasetSize*/, size_t /*byteVariableSized*/)
    {
    ...
    template<class ElemType>
    void GPRNNSequenceWriter<ElemType>::Save(std::wstring& outputFile, const Matrix<ElemType>& outputData, const map<int, string>& idx2wrd, const int& nbest)
    {
    ...
    template class GPRNNSequenceWriter<float>;
    template class GPRNNSequenceWriter<double>;
