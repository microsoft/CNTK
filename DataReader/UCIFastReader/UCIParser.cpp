...
void UCIParser<NumType, LabelType>::ParseInit(LPCWSTR fileName, size_t startFeatures, size_t dimFeatures, size_t startLabels, size_t dimLabels, size_t bufferSize, size_t startPosition)
{
    ...
    if (err)
        throw new std::runtime_error("UCIParser::ParseInit - error opening file"); 
    int rc = _fseeki64(m_pFile, 0, SEEK_END);
    if (rc)
        throw new std::runtime_error("UCIParser::ParseInit - error seeking in file");
...
void UCIParser<NumType, LabelType>::SetFilePosition(_int64 position)
{
    ...
    if (rc)
        throw new std::runtime_error("UCIParser::SetFilePosition - error seeking in file");
...
size_t UCIParser<NumType, LabelType>::UpdateBuffer()
{
    ...
    if (bytesRead == 0 && ferror(m_pFile))
        throw new std::runtime_error("UCIParser::UpdateBuffer - error reading file");
    return bytesRead;
}
