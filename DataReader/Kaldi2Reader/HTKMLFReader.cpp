    ...
    template<class ElemType>
        void HTKMLFReader<ElemType>::PrepareForTrainingOrTesting(const ConfigParameters& readerConfig)
        {
...
#endif
            // Kaldi label reader doesn't use this, but its constructor requires it
            ...
            if (!_stricmp(readMethod.c_str(),"blockRandomize"))
            {
                ...
                m_frameSource = new msra::dbn::minibatchutterancesourcemulti(scriptpaths, infilesmulti, labelsmulti, m_featDims, m_labelDims, numContextLeft, numContextRight, randomize, *m_lattices, m_latticeMap, framemode);
