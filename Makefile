...
CNTKLIBRARY:=Cntk.Library-$(CNTK_COMPONENT_VERSION)
...
COMPOSITEDATAREADER:=$(LIBDIR)/Cntk.Reader.CompositeData-$(CNTK_COMPONENT_VERSION).so
...
HTKDESERIALIZERS:=$(LIBDIR)/Cntk.Reader.HTKDeserializer-$(CNTK_COMPONENT_VERSION).so
...
CNTKBINARYREADER:=$(LIBDIR)/Cntk.Reader.Binary-$(CNTK_COMPONENT_VERSION).so
...
CNTKTEXTFORMATREADER:=$(LIBDIR)/Cntk.Reader.TextFormat-$(CNTK_COMPONENT_VERSION).so
...
KALDI2READER:=$(LIBDIR)/Cntk.Reader.Kaldi-$(CNTK_COMPONENT_VERSION).so
...
IMAGEREADER:=$(LIBDIR)/Cntk.Reader.Image-$(CNTK_COMPONENT_VERSION).so
