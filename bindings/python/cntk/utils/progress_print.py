...
class ProgressPrinter(object):
    ...
    def epoch_summary(self, with_metric=False):
        ...
        if self.freq > 0:
            ...
            if with_metric:
                self.___logprint("Finished Epoch [{}]: {}loss = {:0.6f} * {}, metric = {:0.1f}% * {} {:0.3f}s ({:5.1f} samples/s)".format(self.epochs, self.tag, avg_loss, samples, avg_metric*100.0, samples, time_delta, speed))
            else:
                self.___logprint("Finished Epoch [{}]: {}loss = {:0.6f} * {} {:0.3f}s ({:5.1f} samples/s)".format(self.epochs, self.tag, avg_loss, samples, time_delta, speed))
