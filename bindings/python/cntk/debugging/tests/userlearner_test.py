...
def ffnet(optimizer, num_minibatches_to_train, lr_args, learner_kwargs):
    inputs = 2
    outputs = 2
    hidden_dimension = 50
    ...
    lr= learning_rate_schedule(0.125, *lr_args)
...
@pytest.mark.parametrize("lr_args, learner_kwargs", ADDITIONAL_ARGUMENTS)
def test_user_learner(lr_args, learner_kwargs):
    num_minibatches_to_train = 10

    np.random.seed(SEED)
    # sort based on shape (this works because all parameters have different
    # shapes)
    p1 = sorted([p.value for p in ffnet(sgd, num_minibatches_to_train, lr_args, learner_kwargs)], key=lambda x: x.shape)

    np.random.seed(SEED)
    p2 = sorted([p.value for p in ffnet(MySgdNaive, num_minibatches_to_train, lr_args, learner_kwargs)], key=lambda x: x.shape)

    np.random.seed(SEED)
    p3 = sorted([p.value for p in ffnet(MySgdFast, num_minibatches_to_train, lr_args, learner_kwargs)], key=lambda x: x.shape)
