...
def test_ConvTranspose_without_OutputShape(tmpdir, dtype, device_id):
    ...
    with C.default_options(dtype=dtype):
        ...
        conv_trans_model_without_output_shape = C.convolution_transpose(kernel, x, strides=(1, 1), dilation=(1, 1), auto_padding = [False, True, True])
        ...
        if device_id >= 0: # Dilated convolution is not supported on CPU, hence the following test is run only on GPU.
            conv_trans_model_with_dilation = C.convolution_transpose(kernel, x, strides=(1, 1), dilation=(2, 2), auto_padding = [False, True, True])
