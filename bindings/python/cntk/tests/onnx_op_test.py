...
# save numpy ndarray in ONNX tensor format.
def SaveTensorProto(file_path, variable, data, onnx_value_info_proto):
...
def verify_no_input(model, tmpdir, name):
    ...
    loaded_model = None
    loaded_model, _, _, _ = try_save_load_resave_onnx_model(model, tmpdir, name, loaded_model)
...
def verify_one_input(model, data, tmpdir, name, device=None, loaded_model=None, rtol = 1e-05, atol = 1e-08):
    ...
    loaded_model, onnx_model, test_model_path, test_data_path = try_save_load_resave_onnx_model(model, tmpdir, name, loaded_model)
