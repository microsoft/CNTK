...
def test_op_reduce_log_sum(input_data, axis, device_id, precision):
    ...
    from .. import reduce_log_add_exp
    _test_unary_op(precision, device_id, reduce_log_add_exp, input_data,
                   expected_forward, expected_backward, {'axis': axis})
...
def test_op_reduce_all(input_data, axis, device_id, precision):
    ...
    from .. import reduce_sum, reduce_max, reduce_min, reduce_mean, reduce_log_add_exp, reduce_prod
    ...
    ops = [ (reduce_sum,         lambda x:AA(sum(np.sum(xi) for xi in x)),                           lambda x,f:[np.ones_like(xi) for xi in x]),
            ...
            (reduce_log_add_exp, lambda x:AA(np.log(sum(np.sum(np.exp(xi)) for xi in x))),           lambda x,f:[np.exp(xi-f)     for xi in x]),
