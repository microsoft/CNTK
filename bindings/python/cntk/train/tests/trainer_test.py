...
def test_trainer(tmpdir, no_eval_function):
    ...
    lr_per_sample = C.learning_rate_schedule(0.007, ref_mbsize =1)
...
def test_epochsize_wrn_for_parameter_schedule():
    with warnings.catch_warnings(record=True) as w:
        ...
        C.training_parameter_schedule(0.01, ref_mbsize=1, epoch_size=1000)
...
def test_model_not_criterion_subset():
    ...
    lr_schedule = C.learning_rate_schedule(0.003, ref_mbsize =1)
