...
def create_reader(path):
    mb_source = text_format_minibatch_source(path, [ 
                    StreamConfiguration('query',      vocab_size, True, 'S0'), 
                    StreamConfiguration('slotLabels', num_labels, True, 'S2') ], 10000)
    # what's that 10000 at the end?

emb_dim = 150
hidden_dim = 300
def model(input = PlaceHolder()):
    return sequential([
        layers.embedding(shape=embedding_dim),
        layers.recurrence(, go_backwards=False,
        layers.linear(label_dim)
        ])(input)
    embedding_function = 
