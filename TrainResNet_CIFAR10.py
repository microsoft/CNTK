...
from cntk.learners import nesterov
...
abs_path   = os.path.dirname(os.path.abspath(__file__))
data_path  = os.path.join(abs_path, "..", "..", "..", "DataSets", "CIFAR-10")
class Sgd_zzh(UserLearner):
    def __init__(self, parameters, lr_schedule):
        super(Sgd_zzh, self).__init__(parameters, lr_schedule)
        self.m=[]
        self.n=[]
        self.m1=[]
        self.n1=[]
    ...
    def update(self, gradient_values, training_sample_count, sweep_end):
        eta = self.learning_rate() / training_sample_count*100
        ...
        if(self.count==0):
            ...
            for p, g in gradient_values.items():
                # print(self.layer_now)
                P=p.value
                self.m.append((np.zeros(P.shape)))
                self.n.append((np.zeros(P.shape)))
                self.m1.append((np.zeros(P.shape)))
                self.n1.append((np.zeros(P.shape)))
                self.mm.append((np.zeros(P.shape)))
                self.nn.append((np.zeros(P.shape)))
        ...
        for p, g in gradient_values.items():
            ...
            # print('******************************')
            # print(g.to_ndarray().shape)
            # print('******************************')

            self.m1[layer]=u*self.m[layer]+(1-u)*g.to_ndarray()
            #print(self.m1[layer].shape)
            #self.n1[layer]=v*self.n[layer]+(1-v)*np.power(g.to_ndarray(),2)
            self.n1[layer]=np.maximum(v*self.n[layer],np.abs(g.to_ndarray()))
            # self.n1[layer]=v*self.n[layer]+(1-v)*np.power(g.to_ndarray(),2)
            # print(self.n[layer])
            
            self.mm[layer]=self.m1[layer]/(1-np.power(u,self.count))
            self.nn[layer]=self.n1[layer]/(1-np.power(v,self.count))
            # print('******************************')
            # print(self.mm[layer].shape)
            #self.nn[layer]=self.n[layer]/(1-np.power(v,self.count))
            p.value=p.value-self.mm[layer]*eta/(self.n1[layer]+e)
            layer=layer+1
        self.n=self.n1
        self.m=self.m1
        return True
...
def train_and_evaluate(reader_train, reader_test, network_name, epoch_size, max_epochs, profiler_dir=None,
    ...
    #learner = nesterov(z.parameters,lr_schedule,mm_schedule,l2_regularization_weight = l2_reg_weight)
    learner = Sgd_zzh(z.parameters,lr_schedule)
